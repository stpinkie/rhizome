...
|-------|------|---------|---------|-------------|
| `steering_mode` | `string` | `"one-at-a-time"` | `RHIZOME_AGENTS_DEFAULTS_STEERING_MODE` | How the steering queue is drained per poll |
| `max_parallel_turns` | `int` | `1` | `RHIZOME_AGENTS_DEFAULTS_MAX_PARALLEL_TURNS` | Max concurrent turns. `0` or `1` = sequential; `>1` = parallel across sessions |
