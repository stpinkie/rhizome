...
**Cause:** Rhizome now resolves provider/model in two steps:

- If `provider` is set, the `model` field is sent to that provider unchanged.
- If `provider` is omitted, Rhizome infers the provider from the first `/` segment and sends everything after that first `/` as the runtime model ID.
...
**Fix:** In `~/.rhizome/config.json` (or your config path):
