...
Rhizome 在处理每一个请求时，都会在后台执行多个复杂的交互操作——从消息路由和复杂度评估，到工具执行和模型故障适配。能够准确地看到正在发生什么至关重要，这不仅有助于排查潜在问题，也有助于真正理解代理的运作方式。

## 以调试模式启动 Rhizome

要获取代理运行的详细信息（LLM 请求、工具调用、消息路由），可以使用调试标志启动 Rhizome 网关：
...
默认情况下，Rhizome 会在调试日志中截断过长的字符串（例如*系统提示词*或大型 JSON 输出结果），以保持控制台的可读性。
