  ...
  "hooks": {
    ...
    "processes": {
      "py_review_gate": {
        ...
        "env": {
          "RHIZOME_HOOK_LOG_FILE": "/tmp/rhizome-hook-review-gate.log"
        }
      }
    }
  }
...
```bash
tail -f /tmp/rhizome-hook-review-gate.log
```

If you are developing Rhizome itself rather than only validating the protocol, continue with the Go in-process example as well.
...
- Python process example
  Best for understanding the `JSON-RPC over stdio` protocol and verifying the message flow between Rhizome and an external process
...
hook := myhooks.NewExampleLoggerHook(myhooks.ExampleLoggerHookOptions{
    LogFile:   "/tmp/rhizome-hook-example-logger.log",
    LogEvents: true,
})
  ...
  "hooks": {
    ...
    "builtins": {
      "example_logger": {
        ...
        "priority": 10,
        "config": {
          "log_file": "/tmp/rhizome-hook-example-logger.log",
          "log_events": true
        }
      }
    }
...
LOG_EVENTS = os.getenv("RHIZOME_HOOK_LOG_EVENTS", "1").lower() not in {"0", "false", "no"}
LOG_FILE = os.getenv("RHIZOME_HOOK_LOG_FILE", "").strip()
...
Note: `RHIZOME_HOOK_LOG_FILE` has no default. If you do not set it, the script does not write any file logs.
...
- Rhizome starts the external process
...
The host does not currently accept new RPCs initiated by the process hook. In practice, that means an external hook can only respond to Rhizome calls; it cannot call back into the host to send channel messages.
