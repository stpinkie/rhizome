...
本文档展示如何利用 Rhizome 的 hook 系统实现外部插件工具注入，让 LLM 能调用由外部 hook 进程实现的工具。
...
这样，外部 hook 可以完全实现插件工具，无需在 Rhizome 内部注册任何工具。
...
1. Rhizome 发送 `hook.before_llm`，hook 注入 `get_weather` 工具定义
2. LLM 看到工具定义，决定调用 `get_weather(city="北京")`
3. Rhizome 发送 `hook.before_tool`，hook 使用 `respond` action 返回天气数据
...
这些引用由 Rhizome 的 MediaStore 管理，可以：
- 通过 channel 发送给用户
...
3. **与内置工具共存**：不影响 Rhizome 原有工具的正常运行
