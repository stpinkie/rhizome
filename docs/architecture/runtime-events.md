...
Rhizome runtime events are the read-only observation surface for agent, channel, gateway, message bus, and MCP activity. Publishing events and printing logs are separate responsibilities:
...
This preserves the previous behavior: agent turn, LLM, tool, steering, subturn, and error events appear in logs. Channel, gateway, bus, MCP, and mesh events are still published to the runtime event bus, but they are not printed unless configured.
...
```bash
RHIZOME_EVENTS_LOGGING_ENABLED=true
RHIZOME_EVENTS_LOGGING_INCLUDE="gateway.*,channel.lifecycle.*"
RHIZOME_EVENTS_LOGGING_EXCLUDE="gateway.ready"
RHIZOME_EVENTS_LOGGING_MIN_SEVERITY=info
RHIZOME_EVENTS_LOGGING_INCLUDE_PAYLOAD=false
```
...
|| Event | Trigger | Details |
|| ----- | ------- | ------- |
|| `mesh.peer.connected` | The local P2P node connects to a peer. | `peer_id`, `addr` |
|| `mesh.peer.disconnected` | The local P2P node disconnects from a peer. | `peer_id`, `addr` |
|| `mesh.cap.received` | A capability announcement is received from a peer. | `peer_id`, `models_count`, `skills_count`, `agents_count` |
|| `mesh.cap.queried` | A peer queries our capability. | `peer_id` |
|| `mesh.dht.bootstrap.start` | The DHT bootstrap process starts. | `rendezvous` |
|| `mesh.dht.bootstrap.done` | The DHT bootstrap process finishes. | `rendezvous`, `duration_ms` |
|| `mesh.dht.discovered` | The DHT finds a provider for the rendezvous. | `peer_id`, `addrs_count` |
|| `mesh.remote.delegate.start` | A remote delegate request starts. | `peer_id`, `agent_id`, `correlation_id` |
|| `mesh.remote.delegate.end` | A remote delegate request ends. | `peer_id`, `agent_id`, `correlation_id`, `status` |
|| `mesh.remote.spawn.start` | A remote spawn request starts. | `peer_id`, `agent_id`, `correlation_id` |
|| `mesh.remote.spawn.end` | A remote spawn request ends. | `peer_id`, `agent_id`, `correlation_id`, `status` |
|| `mesh.error` | A mesh operation fails. | `stage`, `error`, `peer_id`; severity is `error` |
...
|| `agent.subturn.spawn` | `child_agent_id`, `label`, `parent_turn_id` |
...
Mesh events add safe payload summaries:

|| Event | Summary fields |
|| ----- | -------------- |
|| `mesh.peer.connected` | `peer_id`, `addr` |
|| `mesh.peer.disconnected` | `peer_id`, `addr` |
|| `mesh.cap.received` | `peer_id`, `models_count`, `skills_count`, `agents_count` |
|| `mesh.cap.queried` | `peer_id` |
|| `mesh.dht.bootstrap.start` | `rendezvous` |
|| `mesh.dht.bootstrap.done` | `rendezvous`, `duration_ms` |
|| `mesh.dht.discovered` | `peer_id`, `addrs_count` |
|| `mesh.remote.delegate.start` | `peer_id`, `agent_id`, `correlation_id` |
|| `mesh.remote.delegate.end` | `peer_id`, `agent_id`, `correlation_id`, `status` |
|| `mesh.remote.spawn.start` | `peer_id`, `agent_id`, `correlation_id` |
|| `mesh.remote.spawn.end` | `peer_id`, `agent_id`, `correlation_id`, `status` |
|| `mesh.error` | `stage`, `error`, `peer_id` |
...
- `mesh.*`: P2P peer, capability, DHT, remote spawn/delegate, and mesh error events.
