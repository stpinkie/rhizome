...
Rhizome stores scheduled jobs in the current workspace and can run them either as reminders, full agent turns, or shell commands.

## Schedule Types

Rhizome currently uses three schedule forms in the cron tool:
...
The CLI command `rhizome cron add` currently supports recurring jobs only:
...
When the job fires, Rhizome sends the saved message back through the agent loop as a new agent turn. Use this for scheduled work that may need reasoning, tools, or a generated reply.

### `deliver: true`

When the job fires, Rhizome publishes the saved message directly to the target channel and recipient without agent processing.

The CLI `rhizome cron add --deliver` flag uses this mode.

### `command`

When a cron-tool job includes `command`, Rhizome runs that shell command through the `exec` tool and publishes the command output back to the channel.
...
The current CLI `rhizome cron add` command does not expose a `command` flag.
...
This is not a hard disable switch. If you set `allow_command` to `false`, Rhizome still allows a command job when the caller explicitly passes `command_confirm: true`.
...
to Rhizome can schedule shell commands. Use it only when every enabled remote
...
If `RHIZOME_HOME` is set, the default workspace becomes:

```text
$RHIZOME_HOME/workspace
```

Both the gateway and `rhizome cron` CLI subcommands use the same `cron/jobs.json` file.

Notes:

- one-time `at_seconds` jobs are deleted after they run
- recurring jobs stay in the store until removed
- disabled jobs stay in the store and still appear in `rhizome cron list`
