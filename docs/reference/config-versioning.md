...
Rhizome uses a schema versioning system for `config.json` to ensure smooth upgrades as the configuration format evolves.
...
When you run Rhizome with a V2 config file:

1. **Detection**: Rhizome reads the `version` field and detects V2
...
- **Default**: `~/.rhizome/config.json.20260413.bak`
- **Custom path**: If using `RHIZOME_CONFIG`, backup is created next to that file
- **Security file**: `.security.yml` is also backed up as `.security.yml.YYYYMMDD.bak`

### Downgrade Risk

⚠️ **Important**: Once migrated to V3, the config **cannot** be safely loaded by older Rhizome versions that only support V2.
...
2. Restore the backup:
   ```bash
   cp ~/.rhizome/config.json.20260413.bak ~/.rhizome/config.json
   cp ~/.rhizome/.security.yml.20260413.bak ~/.rhizome/.security.yml  # if it exists
   ```
3. Use a Rhizome version that supports V2 configs
