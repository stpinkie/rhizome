...
A configuração de ferramentas do Rhizome está localizada no campo `tools` do `config.json`.
...
Por padrão, o Rhizome bloqueia os seguintes comandos perigosos:
...
O guarda exec apenas valida o comando de nível superior enviado ao Rhizome. Ele **não** inspeciona recursivamente processos filhos gerados por ferramentas de build ou scripts após o início desse comando.
...
Todas as opções de configuração podem ser substituídas via variáveis de ambiente com o formato `RHIZOME_TOOLS_<SECTION>_<KEY>`:
