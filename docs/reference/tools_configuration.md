...
Rhizome's tools configuration is located in the `tools` field of `config.json`.
...
Before tool results are sent to the LLM, Rhizome can filter sensitive values (API keys, tokens, secrets) from the output. This prevents the LLM from seeing its own credentials.
...
If `range` is omitted, Rhizome performs an unrestricted search.
...
```bash
RHIZOME_TOOLS_EXEC_ENABLED=false
```
...
By default, Rhizome blocks the following dangerous commands:
...
The exec guard only validates the top-level command sent to Rhizome. It does **not** recursively inspect child
...
If you prefer not to edit JSON manually, Rhizome also provides an MCP configuration manager CLI:

- `rhizome mcp add` — add or update a server (supports `--deferred` / `--no-deferred`)
- `rhizome mcp list` — list all configured servers with status and deferred state
- `rhizome mcp show <name>` — show full details and the tool list for one server
- `rhizome mcp test <name>` — connectivity check for one server
- `rhizome mcp remove <name>` — remove a server entry
- `rhizome mcp edit` — open `config.json` in `$EDITOR` for advanced edits
...
All configuration options can be overridden via environment variables with the format `RHIZOME_TOOLS_<SECTION>_<KEY>`:

For example:

- `RHIZOME_TOOLS_WEB_BRAVE_ENABLED=true`
- `RHIZOME_TOOLS_EXEC_ENABLED=false`
- `RHIZOME_TOOLS_EXEC_ENABLE_DENY_PATTERNS=false`
- `RHIZOME_TOOLS_CRON_EXEC_TIMEOUT_MINUTES=10`
- `RHIZOME_TOOLS_MCP_ENABLED=true`
- `RHIZOME_TOOLS_MCP_MAX_INLINE_TEXT_CHARS=16384`

Note: Nested map-style config (for example `tools.mcp.servers.<name>.*`) is configured in `config.json` rather than
environment variables.

For MCP tools, `tools.mcp.max_inline_text_chars` controls how much text result is kept inline in model context. The threshold is counted in Unicode characters (Go runes), not bytes. For example, `16384` means up to 16,384 characters inline, which may occupy more than 16 KB for multibyte text such as CJK. Above this threshold, Rhizome saves the MCP text result as a local artifact in the agent workspace and gives the model a short note plus a structured `[file:...]` artifact path instead of injecting the full payload into context.
