...
Rhizome 使用统一的命令定义来源。启动时会自动将 Telegram 支持的命令（例如 `/start`、`/help`、`/show`、`/list`、`/use`、`/btw`）注册到 Bot 命令菜单，确保菜单展示与实际行为一致。
...
如果 `session_store_path` 为空，会话存储在 `<workspace>/whatsapp/`。运行 `rhizome gateway`；首次运行时在终端扫描 QR 码（WhatsApp → 关联设备）。
...
Rhizome 通过腾讯 iLink 官方 API 支持连接微信个人号。
...
4. 运行 `rhizome gateway`，打开 QQ 与机器人聊天
...
Rhizome 通过 WebSocket/SDK 模式连接飞书 — 无需公网 Webhook URL 或回调服务器。
...
Rhizome 现在将企业微信统一为一个基于 WebSocket 的 AI Bot 渠道。
...
OneBot 是 QQ 机器人的开放协议。Rhizome 通过 WebSocket 连接任何 OneBot v11 兼容实现（如 [Lagrange](https://github.com/LagrangeDev/Lagrange.Core)、[NapCat](https://github.com/NapNeko/NapCatQQ)）。
...
任意 MQTT 客户端均可通过 Broker 与 Rhizome 通信。设备或服务向 Broker 发布请求，Rhizome 订阅后处理并将响应发布回去。
...
用户名和密码存储于 `~/.rhizome/.security.yml`：
...
```
{prefix}/{agent_id}/{client_id}/request    # 客户端 → Rhizome
{prefix}/{agent_id}/{client_id}/response   # Rhizome → 客户端
```
