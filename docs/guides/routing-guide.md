...
In Rhizome, routing has two user-facing parts:
...
If no rule matches, Rhizome falls back to the default agent.
...
- Rhizome resolves the light model at startup; if it is invalid, routing is disabled
