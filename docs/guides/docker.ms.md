...
Anda juga boleh menjalankan Rhizome menggunakan Docker Compose tanpa memasang apa-apa secara setempat.
...
> [!TIP]
> **Pengguna Docker**: Secara lalai, Gateway mendengar pada `127.0.0.1` yang tidak boleh diakses dari host. Jika anda perlu mengakses health endpoint atau mendedahkan port, tetapkan `RHIZOME_GATEWAY_HOST=0.0.0.0` dalam persekitaran anda atau kemas kini `config.json`.

```bash
# 5. Semak log
docker compose -f docker/docker-compose.yml logs -f rhizome-gateway
...
Imej `launcher` merangkumi kedua-dua binari (`rhizome`, `rhizome-launcher`) dan memulakan konsol web secara lalai, yang menyediakan UI berasaskan pelayar untuk konfigurasi dan sembang.
...
```bash
# Tanyakan soalan
docker compose -f docker/docker-compose.yml run --rm rhizome-agent -m "What is 2+2?"
...
> [!TIP]
> Tetapkan API Key anda dalam `~/.rhizome/config.json`. Dapatkan API Key: [Volcengine (CodingPlan)](https://www.volcengine.com/activity/codingplan?utm_campaign=Rhizome&utm_content=Rhizome&utm_medium=devrel&utm_source=OWO&utm_term=Rhizome) (LLM) · [OpenRouter](https://openrouter.ai/keys) (LLM) · [Zhipu](https://open.bigmodel.cn/usercenter/proj-mgmt/apikeys) (LLM). Carian web adalah pilihan — dapatkan [Tavily API](https://tavily.com) percuma (1000 pertanyaan percuma/bulan) atau [Brave Search API](https://brave.com/search/api) (2000 pertanyaan percuma/bulan).
...
> `request_timeout` adalah pilihan dan menggunakan saat. Jika diabaikan atau ditetapkan kepada `<= 0`, Rhizome menggunakan timeout lalai (120s).
