...
> **Novidade?** O Rhizome agora usa uma abordagem de configuração **centrada no modelo**. Basta especificar o formato `vendor/model` (ex.: `zhipu/glm-4.7`) para adicionar novos provedores — **sem necessidade de alteração de código!**
...
> Execute `rhizome auth login --provider anthropic` para colar seu token de API.
...
O Rhizome remove apenas o prefixo externo `litellm/` antes de enviar a requisição, então aliases de proxy como `litellm/lite-gpt4` enviam `lite-gpt4`, enquanto `litellm/openai/gpt-4o` envia `openai/gpt-4o`.

#### Balanceamento de Carga

Configure múltiplos endpoints para o mesmo nome de modelo — o Rhizome fará automaticamente round-robin entre eles:
...
O Rhizome roteia provedores por família de protocolo:
