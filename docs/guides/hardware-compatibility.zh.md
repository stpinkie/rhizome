...
Rhizome 可在多种 Linux 设备上运行。本页面记录了已验证的芯片、产品和开发板。当前完整构建约为 98 MB，守护进程使用约 60 MB 私有内存，因此总内存低于 256 MB 的开发板未经验证。请参阅[最低要求](#5-最低要求)表格了解两层占用。
...
|| 子架构 | 典型芯片 | 备注 |
||--------|----------|------|
|| ARMv6 | [BCM2835](https://www.raspberrypi.com/documentation/computers/processors.html#bcm2835) (Raspberry Pi 1/Zero) | 单核 ARM1176JZF-S；完整守护进程需要 512 MB+ 内存的开发板 |
...
|| 厂商 | 芯片 | 核心 | 备注 |
||------|------|------|------|
|| [SOPHGO (算能)](https://www.sophgo.com/) | SG2002 | C906 @ 1GHz | 256 MB DDR3 片上内存；完整守护进程需要 512 MB+ 内存的开发板 |
...
|| [Canaan (嘉楠)](https://www.canaan-creative.com/) | K230 | Dual C908 @ 1.6GHz | 6 TOPS KPU；CanMV-K230 开发板外置 512 MB 内存 |

### MIPS

|| 厂商 | 芯片 | 备注 |
||------|------|------|
|| MediaTek | [MT7620](https://www.mediatek.com/products/home-networking/mt7620) | MIPS24KEc @ 580MHz；典型 OpenWrt 路由器内存为 256 MB 或更少，未经验证用于完整守护进程 |
...
|| OpenWrt routers | MIPS/ARM 构建；完整守护进程需要 256 MB+ 可用内存和 128 MB+ 存储。许多家用路由器不满足这些要求。 |
...
当前发布版本比原始 PicoClaw 目标更大，因为它们包含 P2P/mesh 支持（libp2p、DHT、git sync）。请使用下面的两个层级作为实用参考。

|| 模式 | 使用场景 | 总 RAM | 可用 RAM | 存储 |
||------|----------|--------|----------|------|
|| **基础** | 一次性 `rhizome agent`、`rhizome onboard` | 256 MB | 128 MB | 128 MB |
|| **完整** | 带 P2P、syncer 和 gateway 的 `rhizome daemon` | 512 MB | 256 MB | 128 MB |

|| 资源 | 最低要求 | 推荐配置 |
||------|----------|----------|
|| CPU | 任意（单核 0.6GHz+） | 四核 1 GHz+ |
