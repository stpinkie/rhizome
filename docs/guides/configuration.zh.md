...
配置文件路径: `~/.rhizome/config.json`

### 环境变量

你可以使用环境变量覆盖默认路径。这对于便携安装、容器化部署或将 rhizome 作为系统服务运行非常有用。这些变量是独立的，控制不同的路径。

| 变量              | 描述                                                                                                                             | 默认路径                  |
|-------------------|-----------------------------------------------------------------------------------------------------------------------------------------|---------------------------|
| `RHIZOME_CONFIG` | 覆盖配置文件的路径。这直接告诉 rhizome 加载哪个 `config.json`，忽略所有其他位置。 | `~/.rhizome/config.json` |
| `RHIZOME_HOME`   | 覆盖 rhizome 数据根目录。这会更改 `workspace` 和其他数据目录的默认位置。          | `~/.rhizome`             |
...
# 在 /opt/rhizome 中存储所有数据运行 rhizome
# 配置将从默认的 ~/.rhizome/config.json 加载
# 工作区将在 /opt/rhizome/workspace 创建
...
也可通过环境变量覆盖：`RHIZOME_LOG_LEVEL=info`

### 工作区布局 (Workspace Layout)

Rhizome 将数据存储在您配置的工作区中（默认：`~/.rhizome/workspace`）：
...
`evolution` 配置块控制 Rhizome 的自进化运行时。启用后，Agent 会把已完成的回合记录为学习记录。在更高模式下，它可以聚类重复出现的成功模式、生成技能草稿，并可选择把已接受的草稿应用到工作区技能中。
...
`turn_profile` 是 `agents.defaults.turn_profile` 下的可选请求上下文策略，用来控制每个新回合是否带入历史、系统提示、技能提示，以及允许调用哪些工具。不写该配置或设置 `"enabled": false` 时，Rhizome 完全保持原逻辑；设置 `"enabled": true` 后，下面的策略会应用到每个新回合。

所有块都使用同一组 `mode`：

| Mode | 含义 |
| --- | --- |
| `default` | 保持 Rhizome 原逻辑。块缺失或 `mode` 缺失都按 `default` 处理。 |
| `off` | 关闭该块。 |
...
| `system_prompt` | 是否注入 Rhizome 默认身份、工作区指令、记忆、运行时上下文和摘要。关闭后仍可使用外部传入的 system prompt。 |
| `skills` | 是否加载技能目录和 active skill 提示。`custom.allow` 只保留列出的技能名。 |
| `tools` | 暴露给模型并允许执行的工具。`custom.allow` 只保留已注册且列出的工具名。 |

当 `system_prompt.mode` 为 `off`、工具仍可见且没有外部 system prompt 时，Rhizome 会复用现有的工具使用规则作为最小兜底提示。如果 `tools.mode` 为 `off`，则不会添加兜底提示。
...
用 **rhizome-launcher** 打开浏览器控制台前需要先使用密码登录。首次启动时打开 `/launcher-setup` 创建 dashboard 登录密码；后续手动登录使用 `/launcher-login`。

- **配置文件**：与 `config.json` 同一目录（若设置了 `RHIZOME_CONFIG`，则与它所指的文件同目录）。启动器专用文件名为 `launcher-config.json`。
...
- **不再支持的鉴权方式**：不再支持 URL token 登录（`?token=...`）、`RHIZOME_LAUNCHER_TOKEN` 和 `Authorization: Bearer` dashboard 鉴权。
...
1. `~/.rhizome/workspace/skills`（工作区）
2. `~/.rhizome/skills`（全局）
3. `<构建时嵌入路径>/skills`（内置）
...
规则按从上到下顺序检查，第一条命中的规则立即生效。若没有规则命中，Rhizome 会回退到默认 agent。
...
Rhizome 默认在沙箱环境中运行。Agent 只能访问配置的工作区内的文件和执行命令。
...
| 选项                    | 默认值                  | 描述                          |
| ----------------------- | ----------------------- | ----------------------------- |
| `workspace`             | `~/.rhizome/workspace` | Agent 的工作目录              |
...
exec 安全守卫仅检查 Rhizome 直接启动的命令行。它不会递归检查由 `make`、`go run`、`cargo`、`npm run` 或自定义构建脚本等开发工具产生的子进程。
...
* 如果需要比内置守卫更强的隔离，请在容器或虚拟机中运行 Rhizome。
...
Rhizome 可以自动执行周期性任务。在工作区创建 `HEARTBEAT.md` 文件：
...
- `RHIZOME_HEARTBEAT_ENABLED=false` 禁用
- `RHIZOME_HEARTBEAT_INTERVAL=60` 更改间隔
...
| `volcengine` | LLM（火山引擎直连）                     | [volcengine.com](https://www.volcengine.com/activity/codingplan?utm_campaign=Rhizome&utm_content=Rhizome&utm_medium=devrel&utm_source=OWO&utm_term=Rhizome) |
...
> **新特性：** Rhizome 现在优先推荐显式 `provider` + 原生 `model` 的配置方式，例如 `"provider": "zhipu", "model": "glm-4.7"`。如果未设置 `provider`，旧的单字段 `provider/model` 写法仍然兼容。
...
| **火山引擎 (豆包)**     | `volcengine`      | `https://ark.cn-beijing.volces.com/api/v3`          | OpenAI    | [获取](https://www.volcengine.com/activity/codingplan?utm_campaign=Rhizome&utm_content=Rhizome&utm_medium=devrel&utm_source=OWO&utm_term=Rhizome) |
...
- 如果设置了 `provider`，Rhizome 会将 `model` 原样发送。
- 如果未设置 `provider`，Rhizome 会把 `model` 第一个 `/` 之前的字段当作 provider，并把第一个 `/` 之后的全部内容当作最终模型 ID。
...
Telegram 旧环境变量仍兼容：`RHIZOME_CHANNELS_TELEGRAM_STREAMING_ENABLED`、`RHIZOME_CHANNELS_TELEGRAM_STREAMING_THROTTLE_SECONDS`、`RHIZOME_CHANNELS_TELEGRAM_STREAMING_MIN_GROWTH_CHARS`。这些环境变量只作用于 Telegram settings，不会开启或修改 Pico 的 `settings.streaming`。

失败处理保持保守：如果还没有任何可见 chunk 就失败，Rhizome 会回退到普通 `Chat()` 路径重试一次；如果已经有 chunk 展示给用户，则不会再发送一条非流式最终答案，避免界面重复输出。
...
> 运行 `rhizome auth login --provider anthropic` 粘贴 API Token。
...
显式设置 `provider` 后，Rhizome 会把 `openai/gpt-oss-20b` 原样发送给 LM Studio。旧的兼容写法 `"model": "lmstudio/openai/gpt-oss-20b"` 在未设置 `provider` 时也会解析成相同的上游模型 ID。
...
显式设置 `provider` 后，Rhizome 会将 `model` 原样发送。因此 `"provider": "litellm", "model": "lite-gpt4"` 会发送 `lite-gpt4`，而 `"provider": "litellm", "model": "openai/gpt-4o"` 会发送 `openai/gpt-4o`。旧的兼容写法 `litellm/lite-gpt4` 和 `litellm/openai/gpt-4o` 在未设置 `provider` 时也会得到相同结果。

</details>

#### 负载均衡

为同一模型名称配置多个端点，Rhizome 会自动轮询：
...
Rhizome 的 runtime events 会覆盖 agent、channel、gateway、message bus 和 MCP 等运行时组件。默认只打印 `agent.*` 事件，其他事件仍会发布到 runtime event bus，但不会进入日志。
...
Rhizome 通过 `cron` 工具支持 cron 风格的定时任务。Agent 可以设置、列出和取消在指定时间触发的提醒或周期性任务。
...
定时任务在重启后持久保存，存储于 `~/.rhizome/workspace/cron/`。
