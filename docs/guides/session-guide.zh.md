...
Rhizome 的 Session 决定了哪些消息会共享同一段对话历史。
...
`session.dimensions` 只是添加更细的隔离维度，Rhizome 仍然保留一层基础隔离：
...
`session.identity_links` 适合处理这种场景：同一个人可能会以多个原始 sender ID 出现，但你希望 Rhizome 把它们视为同一个发送者身份。
...
这是当前实现下的预期行为。
即使使用了 `sender`，Rhizome 仍然会按 channel 做基础隔离。
...
这属于正常兼容行为。
Rhizome 在迁移到新的 opaque canonical key 时，仍会兼容旧的 `agent:...` session key。
