...
# Utiliser le fournisseur Antigravity dans Rhizome

Ce guide explique comment configurer et utiliser le fournisseur **Antigravity** (Google Cloud Code Assist) dans Rhizome.
...
```bash
rhizome auth login --provider antigravity
```

### Authentification manuelle (Headless/VPS)
Si vous exécutez Rhizome sur un serveur (Coolify/Docker) et ne pouvez pas accéder à `localhost`, suivez ces étapes :
...
6.  **Collez-la dans le terminal** où Rhizome attend.

Rhizome extraira automatiquement le code d'autorisation et terminera le processus.
...
### Changer de modèle
Vous pouvez modifier le modèle par défaut dans `~/.rhizome/config.json` ou le remplacer via le CLI :

```bash
# Remplacer pour une seule commande
rhizome agent -m "Hello" --model claude-opus-4-6-thinking
```
...
1.  **Variables d'environnement** :
    *   `RHIZOME_AGENTS_DEFAULTS_MODEL=gemini-flash`
2.  **Persistance de l'authentification** :
    ...
    ```bash
    scp ~/.rhizome/auth.json user@your-server:~/.rhizome/
    ```
...
*   **429 Limite de débit** : Antigravity a des quotas stricts. Rhizome affichera le « temps de réinitialisation » dans le message d'erreur si vous atteignez une limite.
*   **404 Non trouvé** : Assurez-vous d'utiliser un ID de modèle provenant de la liste `rhizome auth models`. Utilisez l'ID court (par ex. `gemini-3-flash`) et non le chemin complet.
