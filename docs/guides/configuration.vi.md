...
File cấu hình: `~/.rhizome/config.json`

### Biến Môi Trường

Bạn có thể ghi đè các đường dẫn mặc định bằng biến môi trường. Điều này hữu ích cho cài đặt portable, triển khai container, hoặc chạy rhizome như dịch vụ hệ thống. Các biến này độc lập và kiểm soát các đường dẫn khác nhau.

| Biến              | Mô tả                                                                                                                             | Đường Dẫn Mặc Định       |
|-------------------|-----------------------------------------------------------------------------------------------------------------------------------------|---------------------------|
| `RHIZOME_CONFIG` | Ghi đè đường dẫn đến file cấu hình. Chỉ định trực tiếp cho rhizome file `config.json` nào cần tải, bỏ qua tất cả vị trí khác. | `~/.rhizome/config.json` |
| `RHIZOME_HOME`   | Ghi đè thư mục gốc cho dữ liệu rhizome. Thay đổi vị trí mặc định của `workspace` và các thư mục dữ liệu khác.          | `~/.rhizome`             |

**Ví dụ:**

```bash
# Chạy rhizome với file cấu hình cụ thể
# Đường dẫn workspace sẽ được đọc từ trong file cấu hình đó
RHIZOME_CONFIG=/etc/rhizome/production.json rhizome gateway

# Chạy rhizome với tất cả dữ liệu lưu tại /opt/rhizome
# Cấu hình sẽ được tải từ mặc định ~/.rhizome/config.json
# Workspace sẽ được tạo tại /opt/rhizome/workspace
...
Cũng có thể ghi đè bằng biến môi trường: `RHIZOME_LOG_LEVEL=info`

### Bố Cục Workspace

Rhizome lưu trữ dữ liệu trong workspace đã cấu hình (mặc định: `~/.rhizome/workspace`):
...
| `system_prompt` | Chèn identity mặc định của Rhizome, chỉ dẫn workspace, memory, runtime context và summary. System prompt từ request bên ngoài vẫn được dùng khi block này `off`. |
...
Khi `system_prompt.mode` là `off`, tools vẫn hiển thị và không có system prompt bên ngoài, Rhizome dùng lại quy tắc dùng tool hiện có làm prompt fallback tối thiểu. Nếu `tools.mode` là `off`, fallback này không được thêm.
...
Rhizome chạy trong môi trường sandbox mặc định. Agent chỉ có thể truy cập file và thực thi lệnh trong workspace đã cấu hình.
...
| ----------------------- | ----------------------- | ----------------------------------------- |
| `workspace`             | `~/.rhizome/workspace` | Thư mục làm việc của agent               |
...
Guard bảo mật exec chỉ kiểm tra dòng lệnh mà Rhizome khởi chạy trực tiếp. Nó không kiểm tra đệ quy các tiến trình con được tạo bởi công cụ phát triển được phép như `make`, `go run`, `cargo`, `npm run`, hoặc script build tùy chỉnh.
...
* Chạy Rhizome trong container hoặc VM nếu bạn cần cách ly mạnh hơn guard tích hợp.
...
Rhizome có thể thực hiện tác vụ định kỳ tự động. Tạo file `HEARTBEAT.md` trong workspace:
...
| `volcengine` | LLM (Volcengine trực tiếp)              | [volcengine.com](https://www.volcengine.com/activity/codingplan?utm_campaign=Rhizome&utm_content=Rhizome&utm_medium=devrel&utm_source=OWO&utm_term=Rhizome) |
...
> **Tính năng mới:** Rhizome hiện sử dụng cách tiếp cận **lấy mô hình làm trung tâm**. Chỉ cần chỉ định định dạng `vendor/model` (ví dụ: `zhipu/glm-4.7`) để thêm provider mới — **không cần thay đổi code!**
...
| **VolcEngine (Doubao)** | `volcengine/`   | `https://ark.cn-beijing.volces.com/api/v3`          | OpenAI    | [Lấy](https://www.volcengine.com/activity/codingplan?utm_campaign=Rhizome&utm_content=Rhizome&utm_medium=devrel&utm_source=OWO&utm_term=Rhizome) |
| **Antigravity**         | `antigravity/`  | Google Cloud                                        | Custom    | Chỉ OAuth                                                        |

#### Cân Bằng Tải

Cấu hình nhiều endpoint cho cùng tên mô hình — Rhizome sẽ tự động round-robin:
...
Các biến môi trường Telegram cũ vẫn tương thích: `RHIZOME_CHANNELS_TELEGRAM_STREAMING_ENABLED`, `RHIZOME_CHANNELS_TELEGRAM_STREAMING_THROTTLE_SECONDS`, và `RHIZOME_CHANNELS_TELEGRAM_STREAMING_MIN_GROWTH_CHARS`. Chúng chỉ áp dụng cho Telegram settings và không bật hoặc thay đổi `settings.streaming` của Pico.

Hành vi lỗi được giữ thận trọng: nếu streaming lỗi trước khi gửi bất kỳ chunk hiển thị nào, Rhizome thử lại một lần qua đường dẫn `Chat()` thông thường. Nếu đã có chunk hiển thị cho người dùng, Rhizome không gửi thêm một câu trả lời non-streaming thứ hai để tránh lặp output.

### Kiến Trúc Provider

Rhizome định tuyến provider theo họ giao thức:
...
Rhizome hỗ trợ tác vụ theo lịch qua công cụ `cron`.
...
Tác vụ đã lên lịch được lưu trữ bền vững sau khi khởi động lại tại `~/.rhizome/workspace/cron/`.
