...
# Usando o provedor Antigravity no Rhizome

Este guia explica como configurar e usar o provedor **Antigravity** (Google Cloud Code Assist) no Rhizome.
...
6.  **Cole-a de volta no terminal** onde o Rhizome está aguardando.

O Rhizome extrairá automaticamente o código de autorização e completará o processo.
...
### Trocar de modelo
Você pode alterar o modelo padrão em `~/.rhizome/config.json` ou substituí-lo via CLI:
...
*   **429 Limite de taxa**: O Antigravity possui cotas rigorosas. O Rhizome exibirá o "tempo de redefinição" na mensagem de erro se você atingir um limite.
*   **404 Não encontrado**: Certifique-se de que está usando um ID de modelo da lista `rhizome auth models`. Use o ID curto (ex.: `gemini-3-flash`) e não o caminho completo.
