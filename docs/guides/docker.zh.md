...
您也可以使用 Docker Compose 运行 Rhizome，无需在本地安装任何环境。
...
> [!TIP]
> **Docker 用户**: 默认情况下, Gateway 监听 `127.0.0.1`，该端口不会暴露到容器外。如果需要通过端口映射访问健康检查接口，请在环境变量中设置 `RHIZOME_GATEWAY_HOST=0.0.0.0` 或修改 `config.json`。
...
`launcher` 镜像包含两个二进制文件（`rhizome`、`rhizome-launcher`），默认启动 Web 控制台，提供基于浏览器的配置和聊天界面。
...
```bash
# 提问
docker compose -f docker/docker-compose.yml run --rm rhizome-agent -m "2+2 等于几？"
...
> [!TIP]
> 在 `~/.rhizome/config.json` 中设置您的 API Key。获取 API Key: [火山引擎 (CodingPlan)](https://www.volcengine.com/activity/codingplan?utm_campaign=Rhizome&utm_content=Rhizome&utm_medium=devrel&utm_source=OWO&utm_term=Rhizome) (LLM) · [OpenRouter](https://openrouter.ai/keys) (LLM) · [Zhipu (智谱)](https://open.bigmodel.cn/usercenter/proj-mgmt/apikeys) (LLM)。网络搜索是 **可选的** — 获取免费的 [Tavily API](https://tavily.com) (每月 1000 次免费查询) 或 [Brave Search API](https://brave.com/search/api) (每月 2000 次免费查询)。
...
**2. 配置 (Configure)** (`~/.rhizome/config.json`)
...
> `request_timeout` 为可选项，单位为秒。若省略或设置为 `<= 0`，Rhizome 使用默认超时（120 秒）。
