...
Rhizome は幅広い Linux デバイスで動作します。このページでは、検証済みのチップ、製品、開発ボードを記録しています。現在のフルビルドは約 98 MB で、デーモンは約 60 MB のプライベートメモリを使用するため、合計 RAM が 256 MB 未満のボードは検証対象外です。2 段階のフットプリントについては、[最小要件](#5-最小要件)の表を参照してください。
...
||--------------------|----------------|------|
|| ARMv6 | [BCM2835](https://www.raspberrypi.com/documentation/computers/processors.html#bcm2835) (Raspberry Pi 1/Zero) | シングルコア ARM1176JZF-S；フルデーモンには 512 MB+ のボードが必要 |
...
|| ベンダー | チップ | コア | 備考 |
||----------|--------|------|------|
|| [SOPHGO (算能)](https://www.sophgo.com/) | SG2002 | C906 @ 1GHz | 256 MB DDR3 オンチップ；フルデーモンには 512 MB+ のボードが必要 |
...
|| [Canaan (嘉楠)](https://www.canaan-creative.com/) | K230 | Dual C908 @ 1.6GHz | 6 TOPS KPU；CanMV-K230 ボードは 512 MB の外部 RAM を追加 |

### MIPS

|| ベンダー | チップ | 備考 |
||----------|--------|------|
|| MediaTek | [MT7620](https://www.mediatek.com/products/home-networking/mt7620) | MIPS24KEc @ 580MHz；一般的な OpenWrt ルーターは 256 MB 以下で、フルデーモンは未検証 |
...
|| OpenWrt routers | MIPS/ARM ビルド；フルデーモンには 256 MB+ の空き RAM と 128 MB+ のストレージが必要です。多くの家庭用ルーターはこれらの要件を満たしていません。 |
...
現在のリリースビルドは、P2P/mesh サポート（libp2p、DHT、git sync）を含むため、元の PicoClaw ターゲットより大きくなっています。以下の 2 段階を実用的なガイドとして使用してください。

|| モード | 用途 | 合計 RAM | 空き RAM | ストレージ |
||--------|------|----------|----------|------------|
|| **基本** | ワンショットの `rhizome agent`、`rhizome onboard` | 256 MB | 128 MB | 128 MB |
|| **フル** | P2P、syncer、gateway 付きの `rhizome daemon` | 512 MB | 256 MB | 128 MB |

|| リソース | 最小 | 推奨 |
||----------|------|------|
|| CPU | 任意（シングルコア 0.6GHz 以上） | クアッドコア 1 GHz 以上 |
