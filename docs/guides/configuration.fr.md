...
Fichier de configuration : `~/.rhizome/config.json`

### Variables d'Environnement

Vous pouvez remplacer les chemins par défaut à l'aide de variables d'environnement. Ceci est utile pour les installations portables, les déploiements conteneurisés ou l'exécution de Rhizome en tant que service système. Ces variables sont indépendantes et contrôlent des chemins différents.

| Variable          | Description                                                                                                                             | Chemin par défaut         |
|-------------------|-----------------------------------------------------------------------------------------------------------------------------------------|---------------------------|
| `RHIZOME_CONFIG` | Remplace le chemin vers le fichier de configuration. Indique directement à Rhizome quel `config.json` charger, en ignorant tous les autres emplacements. | `~/.rhizome/config.json` |
| `RHIZOME_HOME`   | Remplace le répertoire racine des données Rhizome. Change l'emplacement par défaut du `workspace` et des autres répertoires de données. | `~/.rhizome`             |
...
Peut également être surchargé via la variable d'environnement : `RHIZOME_LOG_LEVEL=info`

### Structure du Workspace

Rhizome stocke les données dans votre workspace configuré (par défaut : `~/.rhizome/workspace`) :
...
`turn_profile` est une politique facultative sous `agents.defaults.turn_profile` pour contrôler le contexte chargé par chaque nouveau tour : historique, prompt système, prompts de skills et outils autorisés. Sans cette configuration, ou avec `"enabled": false`, Rhizome garde son comportement normal. Avec `"enabled": true`, la politique ci-dessous s'applique à chaque nouveau tour.

Tous les blocs utilisent les mêmes valeurs de `mode` :

| Mode | Signification |
| --- | --- |
| `default` | Garde le comportement normal de Rhizome. Un bloc absent ou sans `mode` vaut `default`. |
...
| `system_prompt` | Injection de l'identité Rhizome, des instructions de l'espace de travail, de la mémoire, du contexte d'exécution et du résumé. Les prompts système externes restent autorisés quand ce bloc est `off`. |
...
Quand `system_prompt.mode` vaut `off`, que des outils restent visibles et qu'aucun prompt système externe n'est fourni, Rhizome réutilise sa règle existante d'utilisation des outils comme prompt minimal de secours. Si `tools.mode` vaut `off`, ce prompt de secours n'est pas ajouté.
...
Rhizome s'exécute dans un environnement sandboxé par défaut. L'agent ne peut accéder aux fichiers et exécuter des commandes que dans le workspace configuré.
...
| ----------------------- | ----------------------- | ------------------------------------------------- |
| `workspace`             | `~/.rhizome/workspace` | Répertoire de travail de l'agent                  |
...
Le garde de sécurité exec n'inspecte que la ligne de commande lancée directement par Rhizome. Il n'inspecte pas récursivement les processus enfants générés par les outils de développement autorisés tels que `make`, `go run`, `cargo`, `npm run` ou les scripts de build personnalisés.
...
* Exécutez Rhizome dans un conteneur ou une VM si vous avez besoin d'une isolation plus forte que celle fournie par le garde intégré.
...
Rhizome peut effectuer des tâches périodiques automatiquement. Créez un fichier `HEARTBEAT.md` dans votre workspace :
...
* `RHIZOME_HEARTBEAT_ENABLED=false` pour désactiver
* `RHIZOME_HEARTBEAT_INTERVAL=60` pour changer l'intervalle
...
> **Nouveauté :** Rhizome utilise désormais une approche **centrée sur le modèle**. Spécifiez simplement le format `vendor/model` (ex. `zhipu/glm-4.7`) pour ajouter de nouveaux providers — **aucune modification de code requise !**
...
| **VolcEngine (Doubao)** | `volcengine/`   | `https://ark.cn-beijing.volces.com/api/v3`          | OpenAI    | [Obtenir](https://www.volcengine.com/activity/codingplan?utm_campaign=Rhizome&utm_content=Rhizome&utm_medium=devrel&utm_source=OWO&utm_term=Rhizome) |
| **Antigravity**         | `antigravity/`  | Google Cloud                                        | Custom    | OAuth uniquement                                                 |

#### Équilibrage de Charge

Configurez plusieurs endpoints pour le même nom de modèle — Rhizome effectuera automatiquement un round-robin :
...
Les anciennes variables d'environnement Telegram restent compatibles : `RHIZOME_CHANNELS_TELEGRAM_STREAMING_ENABLED`, `RHIZOME_CHANNELS_TELEGRAM_STREAMING_THROTTLE_SECONDS` et `RHIZOME_CHANNELS_TELEGRAM_STREAMING_MIN_GROWTH_CHARS`. Elles s'appliquent uniquement aux settings Telegram et n'activent ni ne modifient `settings.streaming` de Pico.

Le comportement d'échec est volontairement conservateur : si le streaming échoue avant l'envoi d'un chunk visible, Rhizome réessaie une fois via le chemin `Chat()` normal. Si un chunk a déjà été affiché à l'utilisateur, Rhizome n'envoie pas une deuxième réponse non-streaming, afin d'éviter une sortie dupliquée.

### Architecture des Providers

Rhizome route les providers par famille de protocole :
...
Rhizome supporte les tâches planifiées via l'outil `cron`. L'agent peut définir, lister et annuler des rappels ou tâches récurrentes.
...
Les tâches planifiées persistent après redémarrage dans `~/.rhizome/workspace/cron/`.
