...
Rhizome kini menyimpan definisi arahan dalam satu registry bersama. Semasa startup, Telegram akan mendaftarkan arahan bot yang disokong secara automatik (contohnya `/start`, `/help`, `/show`, `/list`, `/use`, `/btw`) supaya menu arahan dan tingkah laku runtime sentiasa selari.
...
Jika pendaftaran arahan gagal (ralat sementara rangkaian/API), saluran tetap akan bermula dan Rhizome akan mencuba semula pendaftaran di latar belakang.
...
Rhizome boleh menyambung ke WhatsApp dalam dua cara:
...
Jika `session_store_path` kosong, sesi akan disimpan dalam `<workspace>/whatsapp/`. Jalankan `rhizome gateway`; pada larian pertama, imbas kod QR yang dipaparkan dalam terminal menggunakan WhatsApp → Linked Devices.
...
Rhizome menyokong tiga jenis integrasi WeCom:
...
Mana-mana client MQTT boleh berkomunikasi dengan Rhizome melalui broker. Peranti atau perkhidmatan menerbitkan permintaan ke broker; Rhizome melanggan, memproses dan menerbitkan respons kembali.
...
Nama pengguna dan kata laluan dalam `~/.rhizome/.security.yml`:
