...
Arquivo de configuração: `~/.rhizome/config.json`

### Variáveis de Ambiente

Você pode substituir os caminhos padrão usando variáveis de ambiente. Isso é útil para instalações portáteis, implantações em contêineres ou execução do rhizome como serviço do sistema. Essas variáveis são independentes e controlam caminhos diferentes.

| Variável          | Descrição                                                                                                                             | Caminho Padrão              |
|-------------------|-----------------------------------------------------------------------------------------------------------------------------------------|---------------------------|
| `RHIZOME_CONFIG` | Substitui o caminho para o arquivo de configuração. Isso indica diretamente ao rhizome qual `config.json` carregar, ignorando todos os outros locais. | `~/.rhizome/config.json` |
| `RHIZOME_HOME`   | Substitui o diretório raiz para dados do rhizome. Isso altera o local padrão do `workspace` e outros diretórios de dados.          | `~/.rhizome`             |

**Exemplos:**

```bash
# Executar rhizome usando um arquivo de configuração específico
...
# Executar rhizome com todos os dados armazenados em /opt/rhizome
# A configuração será carregada do padrão ~/.rhizome/config.json
# O workspace será criado em /opt/rhizome/workspace
...
Também pode ser substituído pela variável de ambiente: `RHIZOME_LOG_LEVEL=info`

### Layout do Workspace

O Rhizome armazena dados no seu workspace configurado (padrão: `~/.rhizome/workspace`):
...
`turn_profile` é uma política opcional em `agents.defaults.turn_profile` para controlar qual contexto cada novo turno carrega: histórico, prompt de sistema, prompts de skills e ferramentas permitidas. Sem essa configuração, ou com `"enabled": false`, o Rhizome mantém o comportamento normal. Com `"enabled": true`, a política abaixo se aplica a cada novo turno.

Todos os blocos usam os mesmos valores de `mode`:

| Mode | Significado |
| --- | --- |
| `default` | Mantém o comportamento normal do Rhizome. Blocos ausentes ou sem `mode` são tratados como `default`. |
...
| `system_prompt` | Injeção da identidade padrão do Rhizome, instruções do workspace, memória, contexto de execução e resumo. Prompts de sistema externos ainda são permitidos quando este bloco está `off`. |
...
Quando `system_prompt.mode` é `off`, ferramentas continuam visíveis e nenhum prompt de sistema externo é fornecido, o Rhizome reutiliza sua regra existente de uso de ferramentas como prompt mínimo de fallback. Se `tools.mode` é `off`, esse fallback não é adicionado.
...
O Rhizome é executado em um ambiente sandbox por padrão. O agente só pode acessar arquivos e executar comandos dentro do workspace configurado.
...
| ----------------------- | ----------------------- | ----------------------------------------- |
| `workspace`             | `~/.rhizome/workspace` | Diretório de trabalho do agente           |
...
O guard de segurança do exec inspeciona apenas a linha de comando que o Rhizome executa diretamente. Ele não inspeciona recursivamente processos filhos gerados por ferramentas de desenvolvimento permitidas como `make`, `go run`, `cargo`, `npm run` ou scripts de build personalizados.
...
* Execute o Rhizome dentro de um contêiner ou VM se precisar de isolamento mais forte do que o guard integrado oferece.
...
O Rhizome pode executar tarefas periódicas automaticamente. Crie um arquivo `HEARTBEAT.md` no seu workspace:
...
* `RHIZOME_HEARTBEAT_ENABLED=false` para desativar
* `RHIZOME_HEARTBEAT_INTERVAL=60` para alterar o intervalo
...
| `volcengine` | LLM (Volcengine direto)                 | [volcengine.com](https://www.volcengine.com/activity/codingplan?utm_campaign=Rhizome&utm_content=Rhizome&utm_medium=devrel&utm_source=OWO&utm_term=Rhizome) |
...
> **Novidade:** Rhizome agora usa uma abordagem **centrada no modelo**. Basta especificar o formato `vendor/model` (ex.: `zhipu/glm-4.7`) para adicionar novos providers — **sem alterações de código!**
...
| **VolcEngine (Doubao)** | `volcengine/`   | `https://ark.cn-beijing.volces.com/api/v3`          | OpenAI    | [Obter](https://www.volcengine.com/activity/codingplan?utm_campaign=Rhizome&utm_content=Rhizome&utm_medium=devrel&utm_source=OWO&utm_term=Rhizome) |
| **Antigravity**         | `antigravity/`  | Google Cloud                                        | Custom    | Somente OAuth                                                    |

#### Balanceamento de Carga

Configure múltiplos endpoints para o mesmo nome de modelo — Rhizome fará round-robin automaticamente:
...
As variáveis de ambiente legadas do Telegram continuam compatíveis: `RHIZOME_CHANNELS_TELEGRAM_STREAMING_ENABLED`, `RHIZOME_CHANNELS_TELEGRAM_STREAMING_THROTTLE_SECONDS` e `RHIZOME_CHANNELS_TELEGRAM_STREAMING_MIN_GROWTH_CHARS`. Elas se aplicam apenas às settings do Telegram e não ativam nem modificam `settings.streaming` do Pico.

O comportamento de falha é intencionalmente conservador: se o streaming falhar antes de qualquer chunk visível ser enviado, o Rhizome tenta novamente uma vez pelo caminho normal `Chat()`. Se um chunk já foi mostrado ao usuário, o Rhizome não envia uma segunda resposta sem streaming, evitando output duplicado.

### Arquitetura de Providers

Rhizome roteia providers por família de protocolo:
...
Rhizome suporta tarefas agendadas via ferramenta `cron`.
...
As tarefas agendadas persistem após reinicializações em `~/.rhizome/workspace/cron/`.
