...
Converse com seu rhizome através do Telegram, Discord, WhatsApp, Matrix, QQ, DingTalk, LINE, WeCom, Feishu, Slack, IRC, OneBot, MQTT ou MaixCam
...
| **Pico**             | ⭐ Fácil           | Canal de protocolo nativo Rhizome                    |                                                                                                                  |
...
O Rhizome agora mantém definições de comandos em um registro compartilhado. Na inicialização, o Telegram registrará automaticamente os comandos de bot suportados (por exemplo `/start`, `/help`, `/show`, `/list`, `/use`, `/btw`) para que o menu de comandos e o comportamento em tempo de execução permaneçam sincronizados.
...
Se o registro de comandos falhar (erros transitórios de rede/API), o canal ainda inicia e o Rhizome tenta novamente o registro em segundo plano.
...
O Rhizome pode se conectar ao WhatsApp de duas formas:
...
Se `session_store_path` estiver vazio, a sessão é armazenada em `<workspace>/whatsapp/`. Execute `rhizome gateway`; na primeira execução, escaneie o QR code impresso no terminal com WhatsApp → Dispositivos Vinculados.
...
O Rhizome suporta conexão com sua conta pessoal do WeChat usando a API oficial Tencent iLink.
...
4. Execute `rhizome gateway` e abra o QQ para conversar com seu bot
...
O Rhizome suporta três tipos de integração WeCom:
...
O Rhizome se conecta ao Feishu via modo WebSocket/SDK — não é necessário URL de webhook público nem servidor de callback.
...
OneBot é um protocolo aberto para bots QQ. O Rhizome se conecta a qualquer implementação compatível com OneBot v11 (ex.: [Lagrange](https://github.com/LagrangeDev/Lagrange.Core), [NapCat](https://github.com/NapNeko/NapCatQQ)) via WebSocket.
...
Qualquer cliente MQTT pode se comunicar com o Rhizome via broker. Dispositivos ou serviços publicam requisições para o broker; o Rhizome assina, processa e publica as respostas de volta.
...
Nome de usuário e senha em `~/.rhizome/.security.yml`:
