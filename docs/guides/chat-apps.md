...
| **Pico**             | ⭐ Easy            | Native Rhizome protocol channel                      |                                                                                                                  |
...
Rhizome now keeps command definitions in one shared registry. On startup, Telegram will automatically register supported bot commands (for example `/start`, `/help`, `/show`, `/list`, `/use`, `/btw`) so command menu and runtime behavior stay in sync.
...
If command registration fails (network/API transient errors), the channel still starts and Rhizome retries registration in the background.
...
Rhizome can connect to WhatsApp in two ways:
...
If `session_store_path` is empty, the session is stored in `<workspace>/whatsapp/`. Run `rhizome gateway`; on first run, scan the QR code printed in the terminal with WhatsApp → Linked Devices.
...
Rhizome supports connecting to your personal WeChat account using the official Tencent iLink API.
...
4. Run `rhizome gateway` and open QQ to chat with your bot
...
Rhizome now exposes WeCom as a single AI Bot channel over WebSocket.
...
OneBot is an open protocol for QQ bots. Rhizome connects to any OneBot v11 compatible implementation (e.g., [Lagrange](https://github.com/LagrangeDev/Lagrange.Core), [NapCat](https://github.com/NapNeko/NapCatQQ)) via WebSocket.
...
Any MQTT client can communicate with Rhizome via a broker. Devices or services publish requests to the broker; Rhizome subscribes, processes them, and publishes responses back.
...
Username and password go in `~/.rhizome/.security.yml`:
