...
File di configurazione: `~/.rhizome/config.json`

### Variabili d'Ambiente

Puoi sovrascrivere i percorsi predefiniti usando variabili d'ambiente. Questo è utile per installazioni portatili, distribuzioni containerizzate, o per eseguire rhizome come servizio di sistema. Queste variabili sono indipendenti e controllano percorsi diversi.

| Variabile         | Descrizione                                                                                                                             | Percorso Predefinito      |
|-------------------|-----------------------------------------------------------------------------------------------------------------------------------------|---------------------------|
| `RHIZOME_CONFIG` | Sovrascrive il percorso al file di configurazione. Indica direttamente a rhizome quale `config.json` caricare, ignorando tutte le altre posizioni. | `~/.rhizome/config.json` |
| `RHIZOME_HOME`   | Sovrascrive la directory radice per i dati di rhizome. Modifica la posizione predefinita del `workspace` e delle altre directory dati.  | `~/.rhizome`             |

**Esempi:**

```bash
# Esegui rhizome usando un file di configurazione specifico
...
# Esegui rhizome con tutti i dati salvati in /opt/rhizome
# La configurazione verrà caricata dal percorso predefinito ~/.rhizome/config.json
# Il workspace verrà creato in /opt/rhizome/workspace
...
Lo streaming del provider usa un double opt-in ed è disattivato per impostazione predefinita. L'agent prova lo streaming solo quando il canale corrente ha `settings.streaming.enabled: true`, l'entry del modello attivo ha `streaming.enabled: true`, e sia il provider sia il canale supportano lo streaming. Se manca una qualsiasi condizione, Rhizome usa il normale percorso di richiesta non streaming.
...
Le variabili d'ambiente legacy di Telegram restano compatibili: `RHIZOME_CHANNELS_TELEGRAM_STREAMING_ENABLED`, `RHIZOME_CHANNELS_TELEGRAM_STREAMING_THROTTLE_SECONDS` e `RHIZOME_CHANNELS_TELEGRAM_STREAMING_MIN_GROWTH_CHARS`. Si applicano solo alle settings Telegram e non attivano né modificano `settings.streaming` di Pico.

Il comportamento in caso di errore è intenzionalmente conservativo: se lo streaming fallisce prima che venga inviato un chunk visibile, Rhizome riprova una volta tramite il normale percorso `Chat()`. Se un chunk è già stato mostrato all'utente, Rhizome non invia una seconda risposta non streaming, evitando output duplicato.

### Struttura del Workspace

Rhizome salva i dati nel workspace configurato (predefinito: `~/.rhizome/workspace`):
...
Se `tools` è omesso nel frontmatter, l'agent riceve il normale set globale dei tool abilitati. Se `tools` è presente, Rhizome registra per quell'agent solo i tool runtime elencati.
...
Quando un agent ha peer spawnabili, Rhizome inietta automaticamente nel suo system prompt un registry strutturato dei peer. Non serve una chiamata aggiuntiva a un tool `list_agents`.
...
Rhizome esegue in un ambiente sandboxed per impostazione predefinita. L'agent può accedere solo ai file ed eseguire comandi all'interno del workspace configurato.
...
| ----------------------- | ----------------------- | ---------------------------------------------------- |
| `workspace`             | `~/.rhizome/workspace` | Directory di lavoro dell'agent                       |
...
Il controllo di sicurezza exec ispeziona solo la riga di comando avviata direttamente da Rhizome. Non ispeziona ricorsivamente i processi figlio generati da strumenti di sviluppo consentiti come `make`, `go run`, `cargo`, `npm run` o script di build personalizzati.
...
* Esegui Rhizome in un container o VM se hai bisogno di un isolamento più forte di quello fornito dal controllo integrato.
...
Rhizome può eseguire task periodici automaticamente. Crea un file `HEARTBEAT.md` nel tuo workspace:
