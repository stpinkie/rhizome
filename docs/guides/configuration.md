...
Config file: `~/.rhizome/config.json`
...
You can override default paths using environment variables. This is useful for portable installations, containerized deployments, or running rhizome as a system service. These variables are independent and control different paths.

| Variable          | Description                                                                                                                             | Default Path              |
|-------------------|-----------------------------------------------------------------------------------------------------------------------------------------|---------------------------|
| `RHIZOME_CONFIG` | Overrides the path to the configuration file. This directly tells rhizome which `config.json` to load, ignoring all other locations. | `~/.rhizome/config.json` |
| `RHIZOME_HOME`   | Overrides the root directory for rhizome data. This changes the default location of the `workspace` and other data directories.          | `~/.rhizome`             |

**Examples:**

```bash
# Run rhizome using a specific config file
# The workspace path will be read from within that config file
RHIZOME_CONFIG=/etc/rhizome/production.json rhizome gateway

# Run rhizome with all its data stored in /opt/rhizome
# Config will be loaded from the default ~/.rhizome/config.json
# Workspace will be created at /opt/rhizome/workspace
...
You can also override this with the environment variable `RHIZOME_LOG_LEVEL`.

### Workspace Layout

Rhizome stores data in your configured workspace (default: `~/.rhizome/workspace`):
...
The `evolution` block controls Rhizome's self-evolution runtime. When enabled, the agent records completed turns as learning records. In higher modes it can group repeated successful patterns, generate skill drafts, and optionally apply accepted drafts into workspace skills.
...
Use `observe` first if you want to inspect learning records without generating skill changes. Use `draft` when you want Rhizome to prepare reviewable improvements. Use `apply` only when you are comfortable letting accepted drafts update workspace skills.

### Request Context Policy

`turn_profile` is an optional request context policy under `agents.defaults.turn_profile`. Leave it unset or set `"enabled": false` to keep Rhizome's normal behavior. When `"enabled": true`, the same policy applies to every new turn.

Each block uses the same `mode` values:

| Mode | Meaning |
| --- | --- |
| `default` | Keep Rhizome's normal behavior for that block. Missing blocks and missing `mode` fields are treated as `default`. |
...
| `system_prompt` | Whether Rhizome injects its default identity, workspace instructions, memory, runtime context, and summary. External request system prompts are still allowed when this is `off`. |
...
When `system_prompt.mode` is `off`, tools are still visible, and no external system prompt is supplied, Rhizome uses its existing tool-use rule as the minimal fallback prompt. If `tools.mode` is `off`, no fallback prompt is added.
...
**rhizome-launcher** serves a browser UI that requires password sign-in first. On first run, open `/launcher-setup` to create the dashboard password. Later manual sign-ins use `/launcher-login`.

- **Config file**: Same directory as `config.json` (or the file pointed to by `RHIZOME_CONFIG`). The launcher-specific file is `launcher-config.json`.
...
- **Unsupported auth paths**: URL token login (`?token=...`), `RHIZOME_LAUNCHER_TOKEN`, and `Authorization: Bearer` dashboard auth are no longer supported.
...
rule matches, Rhizome falls back to the configured default agent.
...
If `tools` is omitted from frontmatter, the agent gets the normal globally enabled tool set. If `tools` is present, Rhizome registers only the listed runtime tools for that agent.
...
When an agent has spawnable peers and can call `spawn`, Rhizome injects a structured agent registry into that agent's system prompt on every turn. No extra `list_agents` tool call is required.
...
Rhizome runs in a sandboxed environment by default. The agent can only access files and execute commands within the configured workspace.
...
| ----------------------- | ----------------------- | ----------------------------------------- |
| `workspace`             | `~/.rhizome/workspace` | Working directory for the agent           |
...
`read_file` has two mutually exclusive implementations selected by config. Rhizome registers exactly one of them at startup:
...
The exec safety guard only inspects the command line Rhizome launches directly. It does not recursively inspect child
...
* Run Rhizome inside a container or VM if you need stronger isolation than the built-in guard provides.
...
Rhizome can perform periodic tasks automatically. Create a `HEARTBEAT.md` file in your workspace:
...
Rhizome supports separating sensitive data (API keys, tokens, secrets) from your main configuration by storing them in a `.security.yml` file.
...
1. Create `~/.rhizome/.security.yml` with your API keys:
```yaml
model_list:
  gpt-5.4:
    api_keys:
...
```bash
chmod 600 ~/.rhizome/.security.yml
```
...
- If `provider` is set, Rhizome sends `model` unchanged.
...
Provider streaming uses a double opt-in and is disabled by default. The agent only tries streaming when the current channel has `settings.streaming.enabled: true`, the active model entry has `streaming.enabled: true`, and both the provider and channel support streaming. If any condition is missing, Rhizome uses the normal non-streaming request path.
...
Legacy Telegram environment variables remain compatible: `RHIZOME_CHANNELS_TELEGRAM_STREAMING_ENABLED`, `RHIZOME_CHANNELS_TELEGRAM_STREAMING_THROTTLE_SECONDS`, and `RHIZOME_CHANNELS_TELEGRAM_STREAMING_MIN_GROWTH_CHARS`. They only apply to Telegram settings and do not enable or modify Pico `settings.streaming`.

Failure behavior is intentionally conservative: if streaming fails before any visible chunk is sent, Rhizome retries once through the normal `Chat()` path. If a chunk has already been shown to the user, Rhizome does not send a second non-streaming answer, because that would duplicate visible output.
...
With explicit `provider`, Rhizome sends `openai/gpt-oss-20b` unchanged to LM Studio. The legacy compatibility form `"model": "lmstudio/openai/gpt-oss-20b"` still resolves to the same upstream model ID when `provider` is omitted.
...
Rhizome supports cron-style scheduled tasks via the `cron` tool. The agent can set, list, and cancel reminders or recurring jobs that trigger at specified times.
...
Scheduled tasks persist across restarts and are stored in `~/.rhizome/workspace/cron/`.

Command cron jobs can execute shell commands. By default, remote channels cannot schedule command jobs. To allow specific remote channels, set `command_allowed_remotes` to entries such as `"telegram"` or `"telegram:1234567890"`; use `"*"` only if every non-empty channel should be allowed. The `"*"` wildcard is potentially dangerous because any remote channel that can talk to Rhizome can schedule shell commands. This does not bypass `allow_command`, `command_confirm`, or exec safety checks.
