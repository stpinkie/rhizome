...
> **新功能！** Rhizome 现在优先推荐显式 `provider` + 原生 `model` 的配置方式，例如 `"provider": "zhipu", "model": "glm-4.7"`。如果未设置 `provider`，旧的单字段 `provider/model` 写法仍然兼容。
...
| **火山引擎（Doubao）** | `volcengine`   | `https://ark.cn-beijing.volces.com/api/v3`          | OpenAI    | [获取密钥](https://www.volcengine.com/activity/codingplan?utm_campaign=Rhizome&utm_content=Rhizome&utm_medium=devrel&utm_source=OWO&utm_term=Rhizome) |
...
| `provider` | string | 否 | 推荐的 provider 标识。设置后，Rhizome 会将 `model` 原样发送给该 provider |
...
Rhizome 按下面的规则解析 `provider` 和最终发给上游的模型 ID：
...
如果没有配置 `voice.model_name`，且存在 Groq API Key，Rhizome 会继续回退到 Groq 转录。
...
> 运行 `rhizome auth login --provider anthropic` 来设置 OAuth 凭证。
...
为同一个模型名称配置多个端点——Rhizome 会自动在它们之间轮询：
...
当你在 Agent 的模型设置里配置 `primary` + `fallbacks` 时，Rhizome 已经支持自动失败切换。
...
如果你在同一模型上启用了 key 级失败切换，Rhizome 会先在该模型的多 key 候选间切换，再继续切到跨模型备选。
