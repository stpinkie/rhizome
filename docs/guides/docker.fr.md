...
Vous pouvez également exécuter Rhizome avec Docker Compose sans rien installer localement.
...
> [!TIP]
> **Utilisateurs Docker** : Par défaut, le Gateway écoute sur `127.0.0.1`, ce qui n'est pas accessible depuis l'hôte. Si vous devez accéder aux endpoints de santé ou exposer des ports, définissez `RHIZOME_GATEWAY_HOST=0.0.0.0` dans votre environnement ou mettez à jour `config.json`.
...
L'image `launcher` inclut les deux binaires (`rhizome`, `rhizome-launcher`) et démarre la console web par défaut, qui fournit une interface navigateur pour la configuration et le chat.
...
> [!TIP]
> Configurez votre clé API dans `~/.rhizome/config.json`. Obtenir des clés API : [Volcengine (CodingPlan)](https://www.volcengine.com/activity/codingplan?utm_campaign=Rhizome&utm_content=Rhizome&utm_medium=devrel&utm_source=OWO&utm_term=Rhizome) (LLM) · [OpenRouter](https://openrouter.ai/keys) (LLM) · [Zhipu](https://open.bigmodel.cn/usercenter/proj-mgmt/apikeys) (LLM). La recherche web est optionnelle — obtenez gratuitement une [API Tavily](https://tavily.com) (1000 requêtes gratuites/mois) ou une [API Brave Search](https://brave.com/search/api) (2000 requêtes gratuites/mois).
...
> `request_timeout` est optionnel et utilise les secondes. S'il est omis ou défini à `<= 0`, Rhizome utilise le timeout par défaut (120s).
