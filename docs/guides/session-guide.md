...
Rhizome sessions decide which messages share the same conversation history.
...
`session.dimensions` adds finer-grained isolation, but Rhizome still keeps a baseline separation by:
...
`session.identity_links` helps when the same user may appear under multiple raw sender IDs and you want Rhizome to treat them as one sender identity.
...
That is expected.
Rhizome still separates sessions by channel even if you use `sender`.
...
Rhizome keeps compatibility with older `agent:...` session keys while moving runtime storage to opaque canonical keys.
