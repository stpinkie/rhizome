...
Rhizome prend en charge le chiffrement des valeurs `api_key` dans les entrées de configuration `model_list`.
...
Exécutez `rhizome onboard` — il vous demande votre phrase secrète et génère la clé SSH,
...
| Chiffré | `enc://<base64>` | Déchiffré au démarrage avec `RHIZOME_KEY_PASSPHRASE` |
...
1. La **phrase secrète** (`RHIZOME_KEY_PASSPHRASE`)
...
|----------|--------|-------------|
| `RHIZOME_KEY_PASSPHRASE` | Oui (pour `enc://`) | Phrase secrète utilisée pour la dérivation de clé |
| `RHIZOME_SSH_KEY_PATH` | Non | Chemin vers la clé privée SSH. Si non défini, détection automatique depuis `~/.ssh/rhizome_ed25519.key` |

### Détection automatique de la clé SSH

Si `RHIZOME_SSH_KEY_PATH` n'est pas défini, Rhizome recherche la clé dédiée :
...
Exécutez `rhizome onboard` pour le générer automatiquement.
...
> **Remarque :** Un fichier de clé SSH est requis pour le chiffrement des identifiants. Si aucune clé n'est trouvée et que `RHIZOME_SSH_KEY_PATH` n'est pas défini, le chiffrement/déchiffrement échouera. Exécutez `rhizome onboard` pour générer la clé automatiquement.

---

## Migration

Étant donné que les seuls éléments secrets sont `RHIZOME_KEY_PASSPHRASE` et le fichier de clé privée SSH, la migration est simple :

1. Copiez le fichier de configuration sur la nouvelle machine.
2. Définissez `RHIZOME_KEY_PASSPHRASE` avec la même valeur.
3. Copiez le fichier de clé privée SSH au même chemin (ou définissez `RHIZOME_SSH_KEY_PATH` vers son nouvel emplacement).
...
- **La phrase secrète et la clé SSH sont toutes deux requises.** La clé SSH agit comme un second facteur — sans elle, le chiffrement/déchiffrement échouera. Exécutez `rhizome onboard` pour générer la clé si elle n'existe pas.
- **La clé SSH est en lecture seule à l'exécution.** Rhizome n'écrit ni ne modifie jamais le fichier de clé SSH.
- **Les clés en texte clair restent prises en charge.** Les configurations existantes sans `enc://` ne sont pas affectées.
- **Le format `enc://` est versionné** via le champ `info` de HKDF (`rhizome-credential-v1`), permettant de futures mises à niveau d'algorithme sans casser les valeurs chiffrées existantes.
