...
**Antigravity** (Google Cloud Code Assist) é um provedor de modelos de IA apoiado pelo Google que oferece acesso a modelos como Claude Opus 4.6 e Gemini através da infraestrutura de nuvem do Google. Este documento fornece um guia completo sobre como a autenticação funciona, como buscar modelos e como implementar um novo provedor no Rhizome.
...
10. [Criando um Novo Provedor no Rhizome](#criando-um-novo-provedor-no-rhizome)
...
Os perfis de autenticação são armazenados em `~/.rhizome/auth.json`:
...
## Criando um Novo Provedor no Rhizome

Os provedores do Rhizome são implementados como pacotes Go em `pkg/providers/`. Para adicionar um novo provedor:
...
Se seu provedor requer OAuth ou autenticação especial, adicione um caso em `cmd/rhizome/internal/auth/helpers.go`:
...
- **Arquivos Fonte:**
  ...
  - `pkg/auth/store.go` - Armazenamento de credenciais de autenticação (`~/.rhizome/auth.json`)
  ...
  - `cmd/rhizome/internal/auth/helpers.go` - Comandos CLI de autenticação
...
- Atualizar tokens OAuth: `rhizome auth login --provider antigravity`
...
- Verificar o armazenamento do perfil de autenticação: `~/.rhizome/auth.json`
- Executar novamente `rhizome auth login --provider antigravity`
