...
Rhizome 支持对 `model_list` 配置条目中的 `api_key` 值进行加密。
...
运行 `rhizome onboard` — 它会提示你输入密码短语并生成 SSH 密钥，
...
| 加密 | `enc://<base64>` | 启动时使用 `RHIZOME_KEY_PASSPHRASE` 解密 |
...
| 变量 | 是否必需 | 描述 |
|------|----------|------|
| `RHIZOME_KEY_PASSPHRASE` | 是（用于 `enc://`） | 用于密钥派生的密码短语 |
| `RHIZOME_SSH_KEY_PATH` | 否 | SSH 私钥路径。如未设置，自动从 `~/.ssh/rhizome_ed25519.key` 检测 |

### SSH 密钥自动检测

如果未设置 `RHIZOME_SSH_KEY_PATH`，Rhizome 会查找专用密钥：
...
运行 `rhizome onboard` 可自动生成该密钥。

`os.UserHomeDir()` 用于跨平台主目录解析（在 Windows 上读取 `USERPROFILE`，在 Unix/macOS 上读取 `HOME`）。

> **注意：** SSH 密钥文件是凭据加密的必要条件。如果未找到密钥且未设置 `RHIZOME_SSH_KEY_PATH`，加密/解密将失败。运行 `rhizome onboard` 可自动生成密钥。

---

## 迁移

由于唯一的密钥材料是 `RHIZOME_KEY_PASSPHRASE` 和 SSH 私钥文件，迁移非常简单：

1. 将配置文件复制到新机器。
2. 将 `RHIZOME_KEY_PASSPHRASE` 设置为相同的值。
3. 将 SSH 私钥文件复制到相同路径（或将 `RHIZOME_SSH_KEY_PATH` 设置为新位置）。
...
- **密码短语和 SSH 密钥都是必需的。** SSH 密钥作为第二因子 — 没有它，加密/解密将失败。如果密钥不存在，运行 `rhizome onboard` 生成。
- **SSH 密钥在运行时为只读。** Rhizome 不会写入或修改 SSH 密钥文件。
- **仍然支持明文密钥。** 不使用 `enc://` 的现有配置不受影响。
- **`enc://` 格式通过版本控制**，通过 HKDF `info` 字段（`rhizome-credential-v1`）实现，允许未来升级算法而不破坏现有加密值。
