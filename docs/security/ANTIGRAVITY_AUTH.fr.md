...
10. [Créer un nouveau fournisseur dans Rhizome](#créer-un-nouveau-fournisseur-dans-rhizome)
...
- Go ≥ 1.26
- Base de code Rhizome (`pkg/providers/` et `pkg/auth/`)
...
Les profils d'authentification sont stockés dans `~/.rhizome/auth.json` :
...
## Créer un nouveau fournisseur dans Rhizome

Les fournisseurs Rhizome sont implémentés en tant que packages Go sous `pkg/providers/`. Pour ajouter un nouveau fournisseur :
...
Si votre fournisseur nécessite OAuth ou une authentification spéciale, ajoutez un cas dans `cmd/rhizome/internal/auth/helpers.go` :
...
- **Fichiers source :**
  ...
  - `pkg/auth/store.go` - Stockage des identifiants d'authentification (`~/.rhizome/auth.json`)
  ...
  - `cmd/rhizome/internal/auth/helpers.go` - Commandes CLI d'authentification
...
- Rafraîchir les jetons OAuth : `rhizome auth login --provider antigravity`
...
- Vérifier le stockage du profil d'authentification : `~/.rhizome/auth.json`
- Relancer `rhizome auth login --provider antigravity`
