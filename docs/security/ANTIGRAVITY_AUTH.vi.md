...
**Antigravity** (Google Cloud Code Assist) là nhà cung cấp mô hình AI được Google hỗ trợ, cung cấp quyền truy cập vào các mô hình như Claude Opus 4.6 và Gemini thông qua hạ tầng đám mây của Google. Tài liệu này cung cấp hướng dẫn đầy đủ về cách xác thực hoạt động, cách lấy danh sách mô hình và cách triển khai nhà cung cấp mới trong Rhizome.
...
9. [Cấu hình](#cấu-hình)
10. [Tạo nhà cung cấp mới trong Rhizome](#tạo-nhà-cung-cấp-mới-trong-rhizome)
...
- Go ≥ 1.26
- Mã nguồn Rhizome (`pkg/providers/` và `pkg/auth/`)
...
Hồ sơ xác thực được lưu trữ trong `~/.rhizome/auth.json`:
...
## Tạo nhà cung cấp mới trong Rhizome

Các nhà cung cấp Rhizome được triển khai dưới dạng gói Go trong `pkg/providers/`. Để thêm nhà cung cấp mới:
...
Nếu nhà cung cấp của bạn yêu cầu OAuth hoặc xác thực đặc biệt, thêm case vào `cmd/rhizome/internal/auth/helpers.go`:
...
- **File nguồn:**
  ...
  - `pkg/auth/store.go` - Lưu trữ thông tin xác thực (`~/.rhizome/auth.json`)
  ...
  - `cmd/rhizome/internal/auth/helpers.go` - Lệnh CLI xác thực
...
- Làm mới token OAuth: `rhizome auth login --provider antigravity`
...
- Kiểm tra lưu trữ hồ sơ xác thực: `~/.rhizome/auth.json`
- Chạy lại `rhizome auth login --provider antigravity`
