...
Rhizome supports separating sensitive data (API keys, tokens, secrets, passwords) from the main configuration by storing them in a `.security.yml` file. This improves security by:
...
```bash
cp security.example.yml ~/.rhizome/.security.yml
```

### Step 2: Fill in your actual values

Edit `~/.rhizome/.security.yml` and replace placeholder values with your actual API keys and tokens.
...
Restart Rhizome and verify it loads correctly:
```bash
rhizome --version
```
...
**For models:**
```bash
export RHIZOME_CHANNELS_TELEGRAM_TOKEN="token-from-env"
```

**For channels:**
```bash
export RHIZOME_CHANNELS_TELEGRAM_TOKEN="token-from-env"
export RHIZOME_CHANNELS_FEISHU_APP_SECRET="secret-from-env"
```

**For web tools:**
```bash
export RHIZOME_TOOLS_WEB_BRAVE_API_KEY="key-from-env"
export RHIZOME_TOOLS_WEB_BAIDU_API_KEY="baidu-key-from-env"
```

Environment variables have the highest priority and will override both `config.json` and `.security.yml` values.

The pattern is: `RHIZOME_<SECTION>_<KEY>_<FIELD>` with underscores separating path segments and converted to uppercase.

## Security Best Practices

1. **Never commit `.security.yml`** to version control
2. **Add to .gitignore**: Ensure `.security.yml` is in your `.gitignore` file
3. **Set file permissions**: `chmod 600 ~/.rhizome/.security.yml`
...
- Verify the file permissions allow reading (`chmod 600 ~/.rhizome/.security.yml`)
...
```bash
cp ~/.rhizome/config.json ~/.rhizome/config.json.backup
```
...
Edit `~/.rhizome/.security.yml` and replace placeholder values with your actual keys.
...
```bash
rm ~/.rhizome/config.json.backup
# Also remove auto-generated date-stamped backups if desired:
rm ~/.rhizome/config.json.20*.bak ~/.rhizome/.security.yml.20*.bak
```

## Advanced: Encrypted API Keys

Rhizome supports encrypting API keys in the security file for additional protection.

### Setup

1. Set a passphrase via environment variable:
```bash
export RHIZOME_CREDENTIAL_PASSPHRASE="your-secure-passphrase"
```
