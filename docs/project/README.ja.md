...
> * **注意:** 完全な `rhizome` バイナリは約 98MB で、デーモンは約 60MB のプライベートメモリを使用します。リソース最適化は機能が安定した後に行う予定です。

## 📢 ニュース

2026-09-07 🌐 **v0.7.0 — Swarm モード**: 信頼ピアの名前付きグループを追加 — 署名エンベロープによる協調（`/rhizome/swarm/1.0.0`）、presence と roster gossip、offer/claim 分散ワークキュー、ワークスペース経由で共有状態を同期する決定的コーディネーター選出、ゴールオーケストレーション（`rhizome swarm run`）、Network ダッシュボードの Swarms パネル、オプションの GossipSub トランスポート。[v0.7.0 リリースノート](../release-notes/v0.7.0.md)（英語）を参照。

2026-09-05–08 🌐 **v0.5.0–v0.6.2 — Mesh**: NAT トラバーサル、非同期リモートタスク、メッシュセキュリティ強化（署名マニフェスト、ACL、監査ログ）、Network ダッシュボード、ピアスコアリング、タスクのフェイルオーバー。リリースノート: [v0.5.0](../release-notes/v0.5.0.md)–[v0.6.2](../release-notes/v0.6.2.md)。

2026-09-01–04 🚀 **v0.4.x — ポータビリティ**: `linux/386`、`windows/386`、`linux/armv7`、ネイティブ Android（`arm64`/`arm`/`386`/`amd64`）ビルド、Linux 3.4 / Android 4.4 向けに強化された P2P トランスポート、ネットワークステータス API、ローカルプロバイダー（Ollama、vLLM、LM Studio）。
...
_*フットプリントの測定は Windows で `CGO_ENABLED=0`、タグ `goolm,stdjson`、`-ldflags "-s -w"` を使用して行われました；strip 後のバイナリは約 98MB です。_
