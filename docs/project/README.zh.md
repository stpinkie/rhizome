...
2026-03-17 🚀 **v0.2.3 发布！** 系统托盘 UI（Windows & Linux）、子 Agent 状态查询 (`spawn_status`)、实验性 Gateway 热重载、Cron 安全门控，以及 2 项安全修复。Rhizome 已达 **25K ⭐**！
...
2026-02-26 🎉 Rhizome 仅 17 天突破 **20K Stars**！频道自动编排和能力接口上线。

2026-02-16 🎉 Rhizome 一周内突破 12K Stars！社区维护者角色和 [路线图](../../ROADMAP.md) 正式发布。
...
也可以从 [GitHub Releases](https://github.com/stpinkie/rhizome/releases) 页面手动下载对应平台的二进制文件。
...
> **Docker / 虚拟机用户：** Gateway 默认监听 `127.0.0.1`。设置 `RHIZOME_GATEWAY_HOST=0.0.0.0` 或使用 `-public` 参数以允许从宿主机访问。
...
macOS 可能会在首次启动时拦截 `rhizome-launcher`，因为它从互联网下载，未经 Mac App Store 公证。

**第一步：** 双击 `rhizome-launcher`，会出现安全警告：

<p align="center">
<img src="../../assets/macos-gatekeeper-warning.jpg" alt="macOS Gatekeeper 警告" width="400">
</p>

> *"rhizome-launcher" 无法打开 — Apple 无法验证 "rhizome-launcher" 不含可能损害 Mac 或危及隐私的恶意软件。*
...
完成这一次操作后，后续启动 `rhizome-launcher` 将不再弹出警告。
...
pkg install proot
termux-chroot ./rhizome onboard   # chroot 提供标准 Linux 文件系统布局
```
...
对于只有 `rhizome` 核心二进制文件的极简环境（无 Launcher UI），可通过命令行和 JSON 配置文件完成所有配置。
...
此命令会创建 `~/.rhizome/config.json` 和工作区目录。

**2. 配置** (`~/.rhizome/config.json`)
...
Rhizome 通过 `model_list` 配置支持 30+ LLM Provider，使用 `协议/模型` 格式：
...
> 日志详细程度通过 `gateway.log_level` 控制（默认：`warn`）。支持的值：`debug`、`info`、`warn`、`error`、`fatal`。也可通过 `RHIZOME_LOG_LEVEL` 环境变量设置。详见[配置指南](../guides/configuration.zh.md#gateway-日志等级)。
...
Rhizome 可以搜索网络以提供最新信息。在 `tools.web` 中配置：
...
Rhizome 内置文件操作、代码执行、定时任务等工具。详情请参阅 [工具配置](../reference/tools_configuration.zh.md)。
...
Rhizome 原生支持 [MCP](https://modelcontextprotocol.io/) — 连接任意 MCP 服务器，通过外部工具和数据源扩展 Agent 能力。
...
通过 CLI 或任何已集成的聊天应用发送一条消息，即可将 Rhizome 连接到 Agent 社交网络。
...
| 命令                       | 说明                   |
| ------------------------- | ---------------------- |
| `rhizome onboard`        | 初始化配置与工作区       |
| `rhizome auth weixin` | 扫码连接微信个人号 |
| `rhizome agent -m "..."` | 与 Agent 对话           |
| `rhizome agent`          | 交互式对话模式           |
| `rhizome gateway`        | 启动网关                |
| `rhizome status`         | 查看状态                |
| `rhizome version`        | 查看版本信息             |
| `rhizome model`          | 查看或切换默认模型       |
| `rhizome cron list`      | 列出所有定时任务         |
| `rhizome cron add ...`   | 添加定时任务             |
| `rhizome cron disable`   | 禁用定时任务             |
| `rhizome cron remove`    | 删除定时任务             |
| `rhizome skills list`    | 列出已安装 Skills        |
| `rhizome skills install` | 安装 Skill              |
| `rhizome migrate`        | 从旧版本迁移数据         |
| `rhizome auth login`     | 认证 Provider           |
...
查看完整的 [社区路线图](https://github.com/stpinkie/rhizome/issues/988) 和 [CONTRIBUTING.md](../../CONTRIBUTING.md)。
