...
Rhizome can run as a Delta Chat bot by launching a local
...
Rhizome can find it automatically; otherwise set `rpc_server_path` to the
exact binary path.
...
The easiest setup is to let Rhizome create a chatmail account in Delta
...
On startup, Rhizome creates the account through `deltachat-rpc-server`, then
stops with an error that contains the generated address. Replace the relay
marker with that full email address and run Rhizome again:
...
`password` is not needed for Rhizome-created chatmail accounts. Omit it when
...
for classic email accounts that Rhizome must configure itself. In that mode,
`password` is a secure field; on first config load it is moved to
`~/.rhizome/.security.yml`, and it can also be set with
`RHIZOME_CHANNELS_DELTACHAT_PASSWORD`.

`display_name` and `avatar_image` are optional profile settings. When present,
Rhizome applies them on every startup, so changing the avatar path in config is
...
| `password` | No | Legacy only; required when Rhizome must configure/reconfigure a classic mailbox itself |
...
| `data_dir` | No | Account database directory. Default: `~/.rhizome/deltachat/<channel-name>` |
...
With `email` set to `@server`, Rhizome creates the chatmail account, prints
the generated full email in the startup error, and exits. Update `email` to that
full address and run Rhizome again. On later runs, Rhizome selects the
configured account by `email`, applies optional profile settings, marks it as a
bot, and starts IO.

With a new `data_dir` plus legacy `password`, Rhizome can still configure a
...
it. On startup Rhizome prints the bot invite link and QR code. Add the bot from
...
| `email is required` | Choose one listed chatmail server, set `email` to a first-run marker such as `@nine.testrun.org`, run `rhizome g`, then replace it with the generated full email |
| `created chatmail account ...` | Replace the `@server` marker in `email` with the generated full email and run Rhizome again |
