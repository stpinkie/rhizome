...
Rhizome supports any MQTT client as a chat channel. Devices or services publish requests to a broker; Rhizome subscribes, processes them, and publishes responses back.

## 🚀 Quick Start

**1. Add the channel to `~/.rhizome/config.json`:**
...
| Segment | Description |
|---------|-------------|
| `prefix` | Topic prefix, configured server-side. Default: `/rhizome` |
| `agent_id` | Rhizome instance identifier, set in `agent_id` config field |
...
Username and password are stored in `~/.rhizome/.security.yml`, not in `config.json`:
...
| `topic_prefix` | `settings` | No | `/rhizome` | Topic namespace prefix |
| `username` | `.security.yml` | No | — | Broker authentication username |
| `password` | `.security.yml` | No | — | Broker authentication password |
| `client_id` | `settings` | No | auto-generated | Paho client ID sent to the broker. Auto-generated as `rhizome-mqtt-{agent_id}-{8-char hex}` if not set; stays fixed for the process lifetime so reconnects reuse the same ID |
...
Rhizome automatically reconnects to the broker if the connection is lost, with a 5-second retry interval. On reconnect, the subscription is re-established automatically. The broker-side client ID stays the same across reconnects so the broker correctly identifies it as the same session.
...
- **client_id vs session ID**: The `client_id` in the topic path is set by your client application and identifies the conversation session. It is separate from the broker-level client ID used by Rhizome's paho connection.
- **Multiple instances**: If you run multiple Rhizome instances against the same broker with the same `agent_id`, set distinct `client_id` values to avoid broker-level conflicts.
