...
Rhizome 支持将任意 MQTT 客户端作为消息渠道。设备或服务向 Broker 发布请求，Rhizome 订阅后处理并将响应发布回去。

## 🚀 快速开始

**1. 在 `~/.rhizome/config.json` 中添加渠道：**
...
| 段 | 说明 |
|----|------|
| `prefix` | Topic 前缀，由服务端配置，默认 `/rhizome` |
| `agent_id` | Rhizome 实例标识，对应配置中的 `agent_id` 字段 |
...
用户名和密码存储于 `~/.rhizome/.security.yml`，不写入 `config.json`：
...
| `topic_prefix` | `settings` | 否 | `/rhizome` | Topic 命名空间前缀 |
| `username` | `.security.yml` | 否 | — | Broker 认证用户名 |
| `password` | `.security.yml` | 否 | — | Broker 认证密码 |
| `client_id` | `settings` | 否 | 自动生成 | 发送给 Broker 的 paho 客户端 ID。未配置时自动生成为 `rhizome-mqtt-{agent_id}-{8位hex}`，进程生命周期内固定不变，断线重连时复用同一 ID |
...
连接断开后 Rhizome 会自动以 5 秒间隔重连 Broker，重连成功后自动重新订阅。断线重连时复用相同的 Broker 客户端 ID，Broker 能正确识别为同一连接。
...
- **client_id 与会话 ID 的区别**：topic 路径中的 `client_id` 由客户端应用自行设置，用于区分会话；它与 Rhizome paho 连接 Broker 时使用的客户端 ID 是两个独立的概念。
- **多实例部署**：若多个 Rhizome 实例使用相同 `agent_id` 连接同一 Broker，需为每个实例配置不同的 `client_id` 以避免 Broker 层面的冲突。
