...
Rhizome prend en charge n'importe quel client MQTT comme canal de messagerie. Les appareils ou services publient des requêtes vers un broker ; Rhizome s'abonne, les traite et publie les réponses en retour.

## 🚀 Démarrage rapide

**1. Ajouter le canal dans `~/.rhizome/config.json` :**
...
| Segment | Description |
|---------|-------------|
| `prefix` | Préfixe de topic configuré côté serveur. Défaut : `/rhizome` |
| `agent_id` | Identifiant de l'instance Rhizome, défini dans le champ `agent_id` |
...
Le nom d'utilisateur et le mot de passe sont stockés dans `~/.rhizome/.security.yml`, pas dans `config.json` :
...
| `topic_prefix` | `settings` | Non | `/rhizome` | Préfixe de l'espace de noms des topics |
...
| `client_id` | `settings` | Non | auto-généré | ID client paho envoyé au broker. Auto-généré sous la forme `rhizome-mqtt-{agent_id}-{8 hex}` ; fixe pour la durée du processus, réutilisé à la reconnexion |
...
Rhizome se reconnecte automatiquement au broker en cas de perte de connexion, avec un intervalle de 5 secondes. L'abonnement est rétabli automatiquement. L'ID client côté broker reste identique à chaque reconnexion.
...
- **client_id vs ID de session** : Le `client_id` dans le chemin du topic est défini par votre application cliente. Il est distinct de l'ID client paho utilisé par Rhizome pour se connecter au broker.
- **Instances multiples** : Si plusieurs instances Rhizome utilisent le même `agent_id` sur le même broker, définir des `client_id` distincts pour éviter les conflits.
