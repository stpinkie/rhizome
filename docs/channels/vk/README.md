...
2. Store the actual token using one of these methods:
   - **Environment variable**: Set `RHIZOME_CHANNELS_VK_TOKEN` environment variable
   - **Secure storage**: Use Rhizome's secure token storage mechanism

Example using environment variable:
```bash
export RHIZOME_CHANNELS_VK_TOKEN="vk1.a.abc123..."
```
...
1. Add the token to your Rhizome configuration
...
VK has a maximum message length of 4000 characters. Rhizome automatically splits longer messages into multiple parts.
...
4. Check Rhizome logs for error messages
