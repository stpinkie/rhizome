...
Rhizome exposes WeCom as a single `channels.wecom` channel built on the official WeCom AI Bot WebSocket API.
This replaces the legacy `wecom`, `wecom_app`, and `wecom_aibot` split with one unified configuration model.

> No public webhook callback URL is required. Rhizome opens an outbound WebSocket connection to WeCom.
...
All fields can be overridden via environment variables with the prefix `RHIZOME_CHANNELS_WECOM_`:
...
- Rhizome maintains an active WeCom turn so streaming replies can continue on the same stream when possible.
...
- Re-run with a larger `--timeout`: `rhizome auth wecom --timeout 10m`
- If the QR code in the terminal is hard to scan, use the **QR Code Link** printed below it to open in a browser.

### QR code expired

- The QR code has a limited validity. Re-run `rhizome auth wecom` to get a fresh one.
