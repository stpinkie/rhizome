...
Rhizome expose WeCom en tant que canal unique `channels.wecom`, basé sur l'API WebSocket officielle WeCom AI Bot.
Ce canal remplace l'ancienne séparation `wecom`, `wecom_app` et `wecom_aibot` par un modèle de configuration unifié.

> Aucune URL de callback webhook publique n'est requise. Rhizome établit une connexion WebSocket sortante vers WeCom.
...
Tous les champs peuvent être remplacés par des variables d'environnement avec le préfixe `RHIZOME_CHANNELS_WECOM_` :

| Variable d'environnement | Champ correspondant |
| ------------------------ | ------------------- |
| `RHIZOME_CHANNELS_WECOM_ENABLED` | `enabled` |
| `RHIZOME_CHANNELS_WECOM_BOT_ID` | `bot_id` |
| `RHIZOME_CHANNELS_WECOM_SECRET` | `secret` |
| `RHIZOME_CHANNELS_WECOM_WEBSOCKET_URL` | `websocket_url` |
| `RHIZOME_CHANNELS_WECOM_SEND_THINKING_MESSAGE` | `send_thinking_message` |
| `RHIZOME_CHANNELS_WECOM_ALLOW_FROM` | `allow_from` |
| `RHIZOME_CHANNELS_WECOM_REASONING_CHANNEL_ID` | `reasoning_channel_id` |

---

## Comportement à l'exécution

- Rhizome maintient un tour WeCom actif pour que les réponses en streaming puissent continuer sur le même flux lorsque c'est possible.
...
- Relancez avec un `--timeout` plus long : `rhizome auth wecom --timeout 10m`
...
- Le QR code a une durée de validité limitée. Relancez `rhizome auth wecom` pour en obtenir un nouveau.
