...
O Rhizome expõe o WeCom como um único canal `channels.wecom`, construído sobre a API WebSocket oficial do WeCom AI Bot.
Isso substitui a antiga separação `wecom`, `wecom_app` e `wecom_aibot` por um modelo de configuração unificado.

> Nenhuma URL de callback webhook pública é necessária. O Rhizome estabelece uma conexão WebSocket de saída para o WeCom.
...
Todos os campos podem ser substituídos via variáveis de ambiente com o prefixo `RHIZOME_CHANNELS_WECOM_`:
...
- O Rhizome mantém um turno WeCom ativo para que as respostas em streaming possam continuar no mesmo fluxo quando possível.
...
- Execute novamente com um `--timeout` maior: `rhizome auth wecom --timeout 10m`
- Se o QR code no terminal for difícil de escanear, use o **Link do QR Code** exibido abaixo dele para abrir no navegador.

### QR code expirado

- O QR code tem validade limitada. Execute novamente `rhizome auth wecom` para obter um novo.
