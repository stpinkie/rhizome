...
Rhizome 将企业微信整合为单一的 `channels.wecom` 渠道，基于腾讯官方企业微信 AI Bot WebSocket API 实现。
原有的 `wecom`、`wecom_app`、`wecom_aibot` 三个独立渠道已合并为统一配置模型。

> 本渠道无需公网 Webhook 回调地址。Rhizome 主动向企业微信建立出站 WebSocket 连接。
...
所有字段均可通过 `RHIZOME_CHANNELS_WECOM_` 前缀的环境变量覆盖：
...
- Rhizome 维护活跃的企业微信 Turn，流式回复尽可能在同一流上继续。
...
- 使用更长的超时重试：`rhizome auth wecom --timeout 10m`
- 终端二维码不清晰时，使用命令打印的**二维码链接**在浏览器中打开。

### 二维码已过期

- 二维码有效期有限，重新运行 `rhizome auth wecom` 获取新二维码。
