...
available list online and pick one), use the 'add' subcommand:
  ...
  rhizome model add --help

Examples:
  rhizome model                    # Show current default model
  rhizome model gpt-5.2           # Set gpt-5.2 as default
  rhizome model claude-sonnet-4.6 # Set claude-sonnet-4.6 as default
  rhizome model local-model       # Set local VLLM server as default
  rhizome model add -b URL -k KEY # Add a model from a custom endpoint
...
func showCurrentModel(cfg *config.Config) {
	...
	fmt.Println("\nTip: 'rhizome model add -b URL -k KEY' adds a model from a custom")
	fmt.Println("     OpenAI-compatible endpoint (see 'rhizome model add --help').")
}
