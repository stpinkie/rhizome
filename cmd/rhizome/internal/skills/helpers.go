...
func skillsInstallBuiltinCmd(workspace string) {
	builtinSkillsDir := "./rhizome/skills"
...
func skillsListBuiltinCmd() {
	...
	builtinSkillsDir := filepath.Join(filepath.Dir(cfg.WorkspacePath()), "rhizome", "skills")
...
func skillsSearchCmd(query string) {
	...
	for _, result := range results {
		...
		if result.RegistryName == "github" {
			fmt.Printf("     Install: rhizome skills install %s\n", result.Slug)
		} else {
			fmt.Printf("     Install: rhizome skills install --registry=%s %s\n", result.RegistryName, result.Slug)
		}
		fmt.Println()
	}
}
