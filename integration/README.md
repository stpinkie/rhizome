...
Rhizome currently uses two related mechanisms:
...
- runs [`TestIntegration_RealConfiguredServer`](../pkg/mcp/manager_real_server_integration_test.go) to verify that Rhizome can connect to a real server, discover tools, invoke one, and validate the response payload
...
- "Rhizome can still connect to a streamable MCP server after transport normalization changes."
