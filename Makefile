...
RHIZOME_HOME?=$(HOME)/.rhizome
WORKSPACE_DIR?=$(RHIZOME_HOME)/workspace
...
# rejects by default.
ANDROID_NDK?=
# API 21 is the lowest level still shipped by current NDKs (r26+ dropped
# API <= 20) and the minimum the Go runtime supports on android.
ANDROID_API?=21
...
## build: Build the rhizome binary for current platform
build: generate
...
## build-launcher: Build the rhizome-launcher (web console) binary
build-launcher:
	@echo "Building rhizome-launcher for $(PLATFORM)/$(ARCH)..."
ifeq ($(OS),Windows_NT)
	@$(POWERSHELL) "New-Item -ItemType Directory -Force -Path '$(BUILD_DIR)' | Out-Null"
	@$(MAKE) -C web build PLATFORM="$(PLATFORM)" ARCH="$(ARCH)" EXT="$(EXT)" OUTPUT="$(CURDIR)/$(BUILD_DIR)/rhizome-launcher-$(PLATFORM)-$(ARCH)$(EXT)" GO_BUILD_TAGS="$(GO_BUILD_TAGS)"
	@$(POWERSHELL) "Copy-Item -LiteralPath '$(BUILD_DIR)/rhizome-launcher-$(PLATFORM)-$(ARCH)$(EXT)' -Destination '$(BUILD_DIR)/rhizome-launcher$(EXT)' -Force"
else
	@mkdir -p $(BUILD_DIR)
	@GOOS=$(PLATFORM) GOARCH=$(ARCH) $(MAKE) -C web build \
		OUTPUT="$(CURDIR)/$(BUILD_DIR)/rhizome-launcher-$(PLATFORM)-$(ARCH)$(EXT)" \
		WEB_GO='$(WEB_GO)' \
		GO_BUILD_TAGS='$(GO_BUILD_TAGS)' \
		LDFLAGS='$(LDFLAGS)'
	@$(LNCMD) rhizome-launcher-$(PLATFORM)-$(ARCH)$(EXT) $(BUILD_DIR)/rhizome-launcher$(EXT)
endif
	@echo "Build complete: $(BUILD_DIR)/rhizome-launcher$(EXT)"
...
build-launcher-android-arm64:
	@echo "Building rhizome-launcher for android/arm64..."
	@mkdir -p $(BUILD_DIR)
...
build-android-bundle: generate
	...
	@rm -rf $(BUILD_DIR)/android-staging
	@echo "All Android builds complete: $(BUILD_DIR)/rhizome-android-universal.zip"
...
## uninstall: Remove rhizome from system
uninstall:
...
## uninstall-all: Remove rhizome and all data
uninstall-all:
	@echo "Removing workspace and skills..."
	@rm -rf $(RHIZOME_HOME)
	@echo "Removed workspace: $(RHIZOME_HOME)"
...
vet: generate
	@packages="$$($(GO) list $(GOFLAGS) ./...)" && \
		$(GO) vet $(GOFLAGS) $$(printf '%s\n' "$$packages" | grep -v '^github.com/stpinkie/rhizome/web/')
	@cd web/backend && $(WEB_GO) vet ./...

## test: Test Go code
test: generate
	@$(GO) test $(GOFLAGS) $$($(GO) list $(GOFLAGS) ./... | grep -v github.com/stpinkie/rhizome/web/)
	@cd web && make test
...
docker-build:
	...
	docker compose -f docker/docker-compose.yml build rhizome-agent rhizome-gateway

## docker-build-full: Build Docker image with full MCP support (Node.js 24)
docker-build-full:
	@echo "Building full-featured Docker image (Node.js 24)..."
	docker compose -f docker/docker-compose.full.yml build rhizome-agent rhizome-gateway
...
## docker-run: Run rhizome gateway in Docker (Alpine-based)
docker-run:
	docker compose -f docker/docker-compose.yml --profile gateway up

## docker-run-full: Run rhizome gateway in Docker (full-featured)
docker-run-full:
	docker compose -f docker/docker-compose.full.yml --profile gateway up

## docker-run-agent: Run rhizome agent in Docker (interactive, Alpine-based)
docker-run-agent:
	docker compose -f docker/docker-compose.yml run --rm rhizome-agent

## docker-run-agent-full: Run rhizome agent in Docker (interactive, full-featured)
docker-run-agent-full:
	docker compose -f docker/docker-compose.full.yml run --rm rhizome-agent

## docker-clean: Clean Docker images and volumes
docker-clean:
	docker compose -f docker/docker-compose.yml down -v
	docker compose -f docker/docker-compose.full.yml down -v
	docker rmi rhizome:latest rhizome:full 2>/dev/null || true


## build-macos-app: Build Rhizome macOS .app bundle (no terminal window)
build-macos-app:build-launcher
	...
	@echo "macOS .app bundle created: $(BUILD_DIR)/Rhizome.app"
...
help:
	...
	@echo "  WORKSPACE_DIR           # Workspace directory (default: ~/.rhizome/workspace)"
