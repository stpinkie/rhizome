...
func TestEnsureOnboardedRunsOnboardWhenConfigMissing(t *testing.T) {
	...
	execCommand = func(name string, args ...string) *exec.Cmd {
		...
		return exec.Command(
			"sh",
			"-c",
			`test "$RHIZOME_CONFIG" = "$EXPECTED_CONFIG_PATH" &&
mkdir -p "$(dirname "$RHIZOME_CONFIG")" &&
printf '{}' > "$RHIZOME_CONFIG"`,
		)
	}
	...
	if len(gotArgs) != 2 || gotArgs[0] != "onboard" || gotArgs[1] != "--non-interactive" {
		t.Fatalf("command args = %#v, want []string{\"onboard\", \"--non-interactive\"}", gotArgs)
	}
