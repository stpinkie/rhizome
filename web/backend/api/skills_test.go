...
func TestHandleListSkills(t *testing.T) {
	...
	oldBuiltin := os.Getenv("RHIZOME_BUILTIN_SKILLS")
	if err := os.Setenv("RHIZOME_BUILTIN_SKILLS", builtinRoot); err != nil {
		t.Fatalf("Setenv(RHIZOME_BUILTIN_SKILLS) error = %v", err)
	}
	defer func() {
		if oldBuiltin == "" {
			_ = os.Unsetenv("RHIZOME_BUILTIN_SKILLS")
		} else {
			_ = os.Setenv("RHIZOME_BUILTIN_SKILLS", oldBuiltin)
		}
	}()
