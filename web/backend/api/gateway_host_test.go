...
func TestBuildWsURLDoesNotTrustOriginWhenProxyOmitsForwardedProto(t *testing.T) {
	...
	req := httptest.NewRequest("GET", "http://launcher.local/api/pico/info", nil)
	req.Host = "fs-952210-xwj.rhizome.lan.sipeed.com"
	req.Header.Set("Origin", "https://fs-952210-xwj.rhizome.lan.sipeed.com")

	if got := h.buildWsURL(req); got != "ws://fs-952210-xwj.rhizome.lan.sipeed.com:80/pico/ws" {
		t.Fatalf(
			"buildWsURL() = %q, want %q",
			got,
			"ws://fs-952210-xwj.rhizome.lan.sipeed.com:80/pico/ws",
		)
	}
}
