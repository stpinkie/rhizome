...
const (
	autoStartEntryName = "RhizomeLauncher"
	launchAgentLabel   = "io.rhizome.launcher"
)
...
func linuxAutoStartPath() string {
	home, _ := os.UserHomeDir()
	return filepath.Join(home, ".config", "autostart", "rhizome-web.desktop")
}
...
func setLinuxAutoStart(enabled bool, exePath string, args []string) error {
	...
	if enabled {
		...
		content := strings.Join([]string{
			...
			"Version=1.0",
			"Name=Rhizome Web",
			"Comment=Start Rhizome Web on login",
