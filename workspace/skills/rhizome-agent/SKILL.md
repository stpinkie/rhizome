---
name: rhizome-agent
description: "Configure, extend, debug, or contribute to Rhizome itself. Use when the task is about Rhizome CLI commands, config.json, gateway, auth, models, skills, MCP servers, cron, routing, sessions, self-evolution, built-in slash commands, or repository internals. Use Rhizome-native workflows, terminology, paths, and configuration."
...
When this skill is active, stay fully subordinate to Rhizome's real architecture:

- Prefer Rhizome commands, config keys, workspace layout, and docs.
- Follow Rhizome source and docs for behavior, naming, and workflows.
- Treat repository code and checked-in docs as the source of truth.

## Quick Start

```bash
# Initialize ~/.rhizome/config.json and ~/.rhizome/workspace
rhizome onboard

# Authenticate a provider
rhizome auth login --provider openai

# Inspect or switch the default model
rhizome model
rhizome model my-default-model
...
```bash
rhizome onboard
rhizome agent [-m MESSAGE] [--session KEY] [--model MODEL] [--debug]
rhizome gateway [--debug] [--no-truncate] [--allow-empty] [--host HOST]
rhizome status
rhizome version
rhizome migrate
```
...
rhizome model
rhizome model <model_name>

# Add a model from an OpenAI-compatible endpoint
rhizome model add --api-base URL --api-key KEY
rhizome model add -b http://localhost:8000/v1 -k dummy -m my-model -n local

# Reset config to factory defaults (preserves sensitive keys)
rhizome config reset
rhizome config reset --force
```

Notes:

- There is **no** general `rhizome config edit` or `rhizome config set`.
- Advanced edits are usually done by editing `~/.rhizome/config.json` directly.

### Authentication

```bash
rhizome auth login --provider openai
rhizome auth login --provider anthropic --setup-token
rhizome auth login --provider antigravity --device-code
rhizome auth models
rhizome auth status
rhizome auth logout --provider openai
rhizome auth weixin
rhizome auth wecom --timeout 10m
```
...
```bash
rhizome skills list
rhizome skills show <name>
rhizome skills search "query"
rhizome skills install owner/repo/path
rhizome skills install --registry clawhub <slug>
rhizome skills remove <name>
rhizome skills list-builtin
rhizome skills install-builtin
```

Skill loading priority is:

1. `~/.rhizome/workspace/skills`
2. `~/.rhizome/skills`
...
rhizome mcp add --deferred github --env-file .env.github -- npx -y @modelcontextprotocol/server-github
rhizome mcp list
rhizome mcp list --status
rhizome mcp show github
rhizome mcp test github
rhizome mcp edit
rhizome mcp remove github
```

Notes:

- `rhizome mcp` is a **configuration manager** for `tools.mcp.servers`.
...
```bash
rhizome cron list
rhizome cron add --name "Daily summary" --message "Summarize today's logs" --cron "0 18 * * *"
rhizome cron add --name "Ping" --message "heartbeat" --every 300 --deliver
rhizome cron enable <job-id>
rhizome cron disable <job-id>
rhizome cron remove <job-id>
```

Notes:

- The current CLI `rhizome cron add` supports recurring jobs only: `--every` or `--cron`.
- One-shot `at_seconds` jobs exist in the cron system, but not as a first-class CLI flag today.

## Providers

Rhizome supports 30+ LLM providers through `model_list`.

Credential patterns in Rhizome are:

- `model_list[].api_keys` for most hosted APIs
- `rhizome auth login --provider ...` for the built-in auth helper flows (`openai`, `anthropic`, `antigravity`)
...
`rhizome auth login` does **not** cover every provider. For most providers, the normal path is adding a `model_list` entry with `provider`, `model`, and `api_keys`.

### Common Provider Matrix

| Provider | `provider` value | Auth path in Rhizome |
| --- | --- | --- |
| OpenAI | `openai` | OAuth helper via `rhizome auth login --provider openai`, or `model_list[].api_keys` |
| Anthropic | `anthropic` | API key in `model_list[].api_keys`, or helper flow via `rhizome auth login --provider anthropic` |
...
| Antigravity | `antigravity` | OAuth helper via `rhizome auth login --provider antigravity` |
...
Rhizome also carries first-class metadata or routing support for additional vendors such as:
...
Rhizome tools are configured under `tools` in `config.json` and registered dynamically at runtime.
...
Rhizome has a first-class subagent model for long-running work, isolated subproblems, and multi-agent specialization.
...
When an agent has the `spawn` tool and at least one allowed peer, Rhizome injects a lightweight agent registry into the system prompt automatically.
...
```text
spawn(
  task="Search the web for the latest Rhizome MCP integration patterns and summarize them.",
  label="mcp-research",
  agent_id="research"
)
```
...
Rhizome can transcribe inbound audio and synthesize outbound speech, but voice setup is model-driven like the rest of the runtime.
...
| ElevenLabs Scribe | `provider: elevenlabs`, `model: scribe_v1` | Uses Rhizome's dedicated ElevenLabs transcription path |
...
- if it resolves to an ElevenLabs model, Rhizome uses the ElevenLabs transcriber
- if it resolves to a Whisper-compatible model, Rhizome uses the Whisper transcription path
- if it resolves to an audio-capable multimodal model, Rhizome can use audio-model transcription
- if `voice.model_name` is omitted, Rhizome still performs compatibility scanning across `model_list` for legacy auto-detected ASR entries
...
| --- | --- | --- |
| OpenAI-compatible speech | `openai/tts-1` | Best-supported path; Rhizome sends an OpenAI-style `/audio/speech` request |
...
- if that is missing, Rhizome can still scan `model_list` for the first API-backed model whose ID contains `tts`
...
- if a provider rejects `response_format`, Rhizome retries once without that field
- `send_tts` is only registered when TTS detection succeeds


## In-Session Slash Commands

Rhizome's shared slash command registry lives under `pkg/commands`.
...
```text
~/.rhizome/config.json          Main config
~/.rhizome/.security.yml       Sensitive values stored outside config.json
~/.rhizome/auth.json           OAuth/token store
~/.rhizome/workspace/          Default workspace
~/.rhizome/workspace/skills/   Workspace skills
~/.rhizome/workspace/sessions/ Session history
~/.rhizome/workspace/cron/     Scheduled jobs store
```
...
```bash
RHIZOME_CONFIG=/path/to/config.json
RHIZOME_HOME=/path/to/rhizome-home
RHIZOME_BUILTIN_SKILLS=/path/to/custom-builtin-skills
RHIZOME_LOG_LEVEL=debug
RHIZOME_GATEWAY_HOST=0.0.0.0
```

Use `RHIZOME_CONFIG` when the user reports "wrong config file" behavior.
Use `RHIZOME_HOME` when the user wants a portable or service-managed install.

## Rhizome-Native Concepts

### Model Configuration

Rhizome is model-centric. The key fields are:
...
- If `provider` is set, Rhizome sends `model` to that provider unchanged.
...
Rhizome only relies on `name` and `description` frontmatter fields for loading and matching.

### MCP Discovery

Rhizome supports always-loaded and deferred MCP tools.

Use:

- `--deferred` when tools should stay hidden until explicitly discovered
- `rhizome mcp show <name>` to inspect active tools
...
Start with the most Rhizome-native path:

1. Check `rhizome status`.
2. Confirm which config file is active.
3. Inspect `agents.defaults.model_name` and `model_list`.
4. Run `rhizome gateway --debug` for runtime visibility.
...
7. For MCP issues, use `rhizome mcp list`, `show`, and `test`.
...
For gateway and runtime debugging, Rhizome writes logs under its home directory:

```text
~/.rhizome/logs/gateway.log
~/.rhizome/logs/gateway_panic.log
```

If `RHIZOME_HOME` is overridden, use:

```text
$RHIZOME_HOME/logs/gateway.log
$RHIZOME_HOME/logs/gateway_panic.log
```

In practice, check these places first:

- `~/.rhizome/logs/` for persisted gateway logs
- the terminal running `rhizome gateway` or `rhizome agent`
- Docker stdout/stderr via `docker compose -f docker/docker-compose.yml logs -f`
- launcher or service logs if Rhizome is being run under another supervisor

Useful controls:

- `rhizome gateway --debug` for detailed runtime logs
- `rhizome gateway --debug --no-truncate` for full prompt/tool payload inspection
- `gateway.log_level` or `RHIZOME_LOG_LEVEL` to raise verbosity to `debug` or `info`

Special case:

- the standalone `rhizome agent` command uses console logging unless `RHIZOME_LOG_FILE` is set explicitly
- process hooks can write JSONL file logs when `RHIZOME_HOOK_LOG_FILE` is set; this is hook-specific and separate from the main gateway log path

## Repository Map

When contributing code, these paths matter most:

- `cmd/rhizome/main.go` — root CLI wiring
- `cmd/rhizome/internal/agent/` — direct CLI agent command
- `cmd/rhizome/internal/gateway/` — gateway startup and flags
- `cmd/rhizome/internal/auth/` — auth flows, QR onboarding
- `cmd/rhizome/internal/model/` — default model switching and `model add`
- `cmd/rhizome/internal/skills/` — install/list/show/remove/search commands
- `cmd/rhizome/internal/mcp/` — MCP CLI configuration manager
- `cmd/rhizome/internal/cron/` — cron CLI
...
- the `provider` and `model` fields use Rhizome's model-centric rules
...
- `rhizome mcp test <name>` succeeds
- `/show mcp <server>` or `rhizome mcp show <name>` exposes tools
...
If the task is code-level rather than user-facing, read the matching package under `cmd/rhizome/internal/`, `pkg/commands/`, `pkg/agent/`, `pkg/skills/`, or `pkg/mcp/` before proposing behavior changes.
