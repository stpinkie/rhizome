...
> **Re-evaluated post-v0.11.0**: items that shipped are marked with their release; the roadmap now tracks only what's actually open. Work is planned in `.todo.md` sprint tracks (GitHub issues are disabled on this repo); upstream PicoClaw issue numbers from the original roadmap are triaged in `docs/project/upstream-issue-triage.md`. Current sprint: **v0.14.0** (`docs/design/v0.14.0-sprint.md` — the Open Agent Work Market arc, **revised mid-implementation**: Tracks 98+106 landed; llama.cpp, an upstream cherry-pick sweep, ACP-over-HTTPS, and the localization tail added; curated index/DHT/reputation + the recorded testnet E2E deferred to the market-graduation sprint (v0.16.0+)). v0.13.0 is tagged; next: **v0.15.0** (`docs/design/v0.15.0-sprint.md` — the Mesh & Swarm Depth arc, Tracks 112–122); then **v0.16.0** (`docs/design/v0.16.0-sprint.md` — Market Graduation, Tracks 123–136).
...
* **Multi-Agent Collaboration**
  ...
  * **Scheduled → v0.15.0 (Tracks 112–122, "Mesh & Swarm Depth")**: requirement-aware dispatch (`--require-model`/`--require-skill` through `PickPeer`/fanout — the task/cost-aware dispatch the v0.12.0 line deferred), inbound capacity controls (`capacity:` rejection class + failover steering), score-weighted claim picks + subtask requirements (`requires`/`tools`/`model`/`timeout` reach `OfferRequest`), restart reconciliation (`running`→`interrupted` run sweep, persisted open offers re-drive on boot) + `swarm run-cancel`/`run-retry`, coordinator/roster diagnostics (`swarm doctor`, epoch freshness), observability depth (persisted `mesh-activity.jsonl`, `mesh/swarm trace` verbs, peer-score per-op splits + half-life decay), dashboard depth, blob resume/dedup, advisory integration CI. Design: `docs/design/v0.15.0-sprint.md`.
...
* **✅ v0.14.0 Track 98**: module stream bridge shipped (`ModuleSpec.Protocols` + catalog schema v4 + loopback bridge + per-module tokens + mesh-trust-gated inbound — any wire-speaking module can use it; works under `--no-gateway`).
* **Scheduled → v0.14.0 Tracks 99–100**: `Capability.ModuleAdverts` merge, and the `rhizome-market` first-party tenant (the work-market module — see §9 Stage 3).
* ✅ **New tenants** (v0.11.0 Track 71): `helios` (Ethereum light client; linux/darwin amd64+arm64, sha256-pinned), `ipfs-kubo` (IPFS node; adds `.zip` extraction, per-GOOS asset templates, `{module_dir}` placeholder, and marker-gated `init_args`/per-launch `setup_args` — catalog schema v2). **Scheduled → v0.14.0 Track 109**: `llama.cpp` local-inference module — `binary_path` layout-preserving extraction, field-fed asset templates (`gpu_backend`), digest-pinned weights fetch; catalog schema v5 (design: `docs/design/llama-cpp-module.md`). Future candidates: ACP agent wrappers.
...
* **Stage 3 — Open Agent Work Market (v0.14.0, Tracks 97–111 revised; opt-in companion module)**: paid agent work between *mutually untrusted* operators — ACP-executed tasks inside sandboxed/containerised agents (prompts in, results out; buyers never run seller code), on-chain escrow settlement via the Stage-2 web3 machinery (adopt-existing contract, survey in `docs/design/escrow-survey.md`), and provider discovery. Ships only as the `rhizome-market` companion module — never in the base package; core carries only generic, inert seams (`acp.runtime` modes + net isolation, module stream bridge + `acp.remote` — ✅ Track 98, `ModuleAdverts` manifest merge, thin `rhizome market` CLI stub). **Revised scope**: the market ships *experimental* — fixture settlement rail default, bound-but-unverified escrow opt-in (Track 104 slimmed), direct-peer discovery via signed-manifest adverts; the ACP-over-HTTPS transport (Track 110) opens buys to non-Rhizome counterparties; curated index, DHT tier, reputation fields, and the recorded testnet E2E move to **v0.16.0 — the market-graduation sprint** (`docs/design/v0.16.0-sprint.md`, Tracks 123–136): Track 105's scope lands as Tracks 124–126 (reputation substrate, curated index, DHT tier), the Track-104 tail as Track 132 (the recorded E2E is the graduation gate), and the Stage-D horizon items pull in as Tracks 128–131 (drawdown, arbitration, TEE posture, portable reputation). Design doc `docs/design/mesh-economy-module.md`. The usage-reporting primitive shipped standalone as v0.13.0 Track 94 (observability + the receipt's usage shape).
