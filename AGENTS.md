...
- Remote-task usage reporting (v0.13.0): peers advertising `allows.usage_report` in their capability manifest get `want_usage` on delegate/spawn/task-submit and return a summed `usage` report (`llm_calls`, `prompt_tokens`, `completion_tokens`, `total_tokens`, `duration_ms`) on the response/result. Unnegotiated calls send and receive today's wire bytes exactly; external ACP agents report no usage. `delegate`, `route`, `scatter`, and `task result` print a `Usage:` line when present; usage also lands in `mesh-tasks.jsonl`, `mesh-audit.jsonl`, and `mesh peer <id>` recent-task detail.
