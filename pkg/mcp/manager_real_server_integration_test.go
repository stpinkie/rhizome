...
//
//	RHIZOME_MCP_REAL_SERVER_JSON='{"enabled":true,"type":"http","url":"http://127.0.0.1:8080/mcp"}'
//
// Optional tool invocation:
//
//	RHIZOME_MCP_REAL_TOOL_NAME=echo
//	RHIZOME_MCP_REAL_TOOL_ARGS_JSON='{"message":"hello"}'
//	RHIZOME_MCP_REAL_EXPECT_SUBSTRING=hello
//
// Stdio subprocess example:
//
//	RHIZOME_MCP_REAL_SERVER_JSON='{"enabled":true,"type":"stdio","command":"npx","args":["-y","@modelcontextprotocol/server-filesystem","."]}'
func TestIntegration_RealConfiguredServer(t *testing.T) {
	if testing.Short() {
		t.Skip("skipping integration test in short mode")
	}

	serverJSON := strings.TrimSpace(os.Getenv("RHIZOME_MCP_REAL_SERVER_JSON"))
	if serverJSON == "" {
		t.Skip("skipping integration test (set RHIZOME_MCP_REAL_SERVER_JSON to enable)")
	}
	...
	if expectedCountRaw := strings.TrimSpace(os.Getenv("RHIZOME_MCP_REAL_EXPECT_TOOL_COUNT")); expectedCountRaw != "" {
		expectedCount, err := strconv.Atoi(expectedCountRaw)
		if err != nil {
			t.Fatalf("invalid RHIZOME_MCP_REAL_EXPECT_TOOL_COUNT %q: %v", expectedCountRaw, err)
		}
	...
	toolName := strings.TrimSpace(os.Getenv("RHIZOME_MCP_REAL_TOOL_NAME"))
	if toolName == "" {
		return
	}

	toolArgs, err := loadRealToolArgs(os.Getenv("RHIZOME_MCP_REAL_TOOL_ARGS_JSON"))
	if err != nil {
	...
	if want := os.Getenv("RHIZOME_MCP_REAL_EXPECT_SUBSTRING"); want != "" && !strings.Contains(textPayload, want) {
