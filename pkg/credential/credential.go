...
//  2. RHIZOME_SSH_KEY_PATH env var
//  3. ~/.ssh/rhizome_ed25519.key (os.UserHomeDir is cross-platform)
package credential
...
const PassphraseEnvVar = "RHIZOME_KEY_PASSPHRASE"

// PassphraseProvider is the function used to retrieve the passphrase for enc://
// credential decryption. It defaults to reading RHIZOME_KEY_PASSPHRASE from the
...
const SSHKeyPathEnvVar = "RHIZOME_SSH_KEY_PATH"

// rhizomeHome is a package-local copy of config.EnvHome. It is kept here to
// avoid a circular import between pkg/credential and pkg/config.
const rhizomeHome = "RHIZOME_HOME"
...
// RHIZOME_SSH_KEY_PATH env var or ~/.ssh/rhizome_ed25519.key.
...
//   - exact match with RHIZOME_SSH_KEY_PATH env var
//   - within the RHIZOME_HOME env var directory
//   - within ~/.ssh/
func allowedSSHKeyPath(path string) bool {
	...
	// Exact match with RHIZOME_SSH_KEY_PATH.
	...
	// Within RHIZOME_HOME.
	if picoHome := os.Getenv(rhizomeHome); picoHome != "" {
	...
	if sshKeyPath == "" {
		return nil, fmt.Errorf(
			"credential: SSH private key is required but not found" +
				" (set RHIZOME_SSH_KEY_PATH or place key at ~/.ssh/rhizome_ed25519.key)")
	}
	if !allowedSSHKeyPath(sshKeyPath) {
		return nil, fmt.Errorf(
			"credential: SSH key path %q is not in an allowed location (RHIZOME_SSH_KEY_PATH, RHIZOME_HOME, or ~/.ssh/)",
			sshKeyPath,
		)
	}
...
//  2. RHIZOME_SSH_KEY_PATH env var
//  3. ~/.ssh/rhizome_ed25519.key (auto-detection)
//
...
// findDefaultSSHKey returns the rhizome-specific SSH key path if it exists.
