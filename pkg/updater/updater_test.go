...
const testReleaseAPIPath = "/api.github.com/repos/sipeed/rhizome/releases/latest"
...
func TestDownloadAndExtractRelease_RetriesTransientAssetFailure(t *testing.T) {
	...
	server = httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		switch r.URL.Path {
		case "/api.github.com/repos/sipeed/rhizome/releases/latest":
	...
	dir, err := DownloadAndExtractRelease(
		server.URL+"/api.github.com/repos/sipeed/rhizome/releases/latest",
		"windows",
		"amd64",
	)
	if err != nil {
