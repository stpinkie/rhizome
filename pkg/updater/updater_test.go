...
const testReleaseAPIPath = "/api.github.com/repos/stpinkie/rhizome/releases/latest"
...
func TestDownloadAndExtractRelease_IntegrationLatestRelease(t *testing.T) {
	if os.Getenv("RHIZOME_INTEGRATION_TESTS") == "" {
		t.Skip("skipping integration test (set RHIZOME_INTEGRATION_TESTS=1 to enable)")
	}
	if testing.Short() {
...
func TestFindAssetInfo_SelectsPreferredAsset(t *testing.T) {
		...
		case testReleaseAPIPath:
			writeReleasePayload(w, testReleasePayload{
				...
				Assets: []testReleaseAsset{
					{
						Name:               "rhizome_Linux_x86_64.zip",
						BrowserDownloadURL: server.URL + "/assets/rhizome_Linux_x86_64.zip",
						Digest:             "sha256:" + strings.Repeat("1", 64),
					},
					{
						Name:               "rhizome_Linux_x86_64.tar.gz",
						BrowserDownloadURL: server.URL + "/assets/rhizome_Linux_x86_64.tar.gz",
						Digest:             "sha256:" + strings.Repeat("2", 64),
					},
					{
						Name:               "rhizome_Windows_x86_64.zip",
						BrowserDownloadURL: server.URL + "/assets/rhizome_Windows_x86_64.zip",
						Digest:             "sha256:" + strings.Repeat("3", 64),
					},
					{
						Name:               "rhizome_Windows_arm64.zip",
						BrowserDownloadURL: server.URL + "/assets/rhizome_Windows_arm64.zip",
	...
	tests := []struct {
	...
	}{
		{
			...
			platform:     "linux",
			arch:         "amd64",
			wantURL:      server.URL + "/assets/rhizome_Linux_x86_64.tar.gz",
		...
		},
		{
			...
			platform:     "windows",
			arch:         "amd64",
			wantURL:      server.URL + "/assets/rhizome_Windows_x86_64.zip",
		...
		},
		{
			...
			platform:     "windows",
			arch:         "arm64",
			wantURL:      server.URL + "/assets/rhizome_Windows_arm64.zip",
...
func TestFindAssetInfo_UsesChecksumAssetWhenDigestMissing(t *testing.T) {
	...
	server = httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/assets/checksums.txt":
			_, _ = io.WriteString(w, checksum+"  rhizome_Windows_x86_64.zip\n")
		case "/assets/rhizome_Windows_x86_64.zip":
	...
	if gotURL != server.URL+"/assets/rhizome_Windows_x86_64.zip" {
		t.Fatalf("assetURL = %q, want %q", gotURL, server.URL+"/assets/rhizome_Windows_x86_64.zip")
	}
...
func TestDownloadAndExtractRelease_ExtractsTarGz(t *testing.T) {
	tarGzContent := buildTestTarGz(t, map[string]string{
		"rhizome_Linux_x86_64/rhizome": "test linux binary payload",
	})
	...
	server = httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/assets/rhizome_Linux_x86_64.tar.gz":
	...
	binPath, err := findBinaryInDir(dir, "rhizome")
	if err != nil {
...
func TestDownloadAndExtractRelease_RetriesTransientAssetFailure(t *testing.T) {
	zipContent := buildTestZip(t, map[string]string{
		"rhizome.exe": "test windows binary payload",
	})
	...
	server = httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		switch r.URL.Path {
		case "/api.github.com/repos/stpinkie/rhizome/releases/latest":
			w.Header().Set("Content-Type", "application/json")
			fmt.Fprintf(
				w,
				`{"tag_name":"v0.2.6","assets":[{"name":"rhizome_Windows_x86_64.zip","browser_download_url":%q,"digest":"sha256:%s"}]}`,
				server.URL+"/assets/rhizome_Windows_x86_64.zip",
				checksum,
			)
		case "/assets/rhizome_Windows_x86_64.zip":
			assetAttempts++
	...
	dir, err := DownloadAndExtractRelease(
		server.URL+"/api.github.com/repos/stpinkie/rhizome/releases/latest",
		"windows",
		"amd64",
	)
	if err != nil {
	...
	bs, err := os.ReadFile(filepath.Join(dir, "rhizome.exe"))
	if err != nil {
		t.Fatalf("ReadFile extracted asset: %v", err)
	}
