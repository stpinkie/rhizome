...
// isRhizomeProcess reads /proc/<pid>/comm to confirm the process name
// contains "rhizome". Returns false when the comm file can be read and
...
func isRhizomeProcess(pid int) bool {
	data, err := os.ReadFile(fmt.Sprintf("/proc/%d/comm", pid))
	if err != nil {
		return true // cannot verify — trust liveness check
	}
	return strings.Contains(strings.TrimSpace(string(data)), "rhizome")
}
