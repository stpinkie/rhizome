...
func TestMatrixLocalpartMentionRegexp(t *testing.T) {
	re := localpartMentionRegexp("rhizome")

	cases := []struct {
		text string
		want bool
	}{
		{text: "@rhizome hello", want: true},
		{text: "hi @rhizome:matrix.org", want: true},
		{
			text: "\u6b22\u8fce\u4e00\u4e0brhizome\u5c0f\u9f99\u867e",
...
func TestStripUserMention(t *testing.T) {
	userID := id.UserID("@rhizome:matrix.org")

	cases := []struct {
		in   string
		want string
	}{
		{in: "@rhizome:matrix.org hello", want: "hello"},
		{in: "@rhizome, hello", want: "hello"},
...
func TestIsBotMentioned(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			...
			msg: event.MessageEventContent{
				...
				Mentions: &event.Mentions{
					UserIDs: []id.UserID{id.UserID("@rhizome:matrix.org")},
				},
			},
			want: true,
		},
		...
		},
		{
			...
			msg: event.MessageEventContent{
				Body:          "hello bot",
				FormattedBody: `<a href="https://matrix.to/#/@rhizome:matrix.org">Rhizome</a> hello`,
			},
			want: true,
		},
		{
			...
			msg: event.MessageEventContent{
				Body:          "hello bot",
				FormattedBody: `<a href="https://matrix.to/#/%40rhizome%3Amatrix.org">Rhizome</a> hello`,
			},
			want: true,
		},
	}
...
func TestDownloadMedia_WritesResponseToTempFile(t *testing.T) {
	...
	client, err := mautrix.NewClient(server.URL, id.UserID("@rhizome:matrix.test"), "")
	if err != nil {
