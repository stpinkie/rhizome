// Package deltachat implements a Rhizome channel for Delta Chat, an
// email-based, end-to-end encrypted messenger.
//
// Rhizome does not link the Delta Chat core directly. Instead it drives a
...
// wire) that Rhizome sets when calling send_msg. Viewtype is normally left empty
...
func parseDeltaChatEmailSetting(value string) (string, bool, error) {
	...
	if email == "" {
		return "", false, fmt.Errorf(
			"deltachat: email is required.\nNext step: choose one of the chatmail servers below and set channel_list.deltachat.settings.email to %q, or use the same @server form with another chatmail relay. Run `rhizome g` again; Rhizome will create the account, print the generated full email address, and stop so you can save that address in the config.\nAvailable chatmail servers:\n%s",
...
func (c *DeltaChatChannel) createChatmailBootstrapAccount(ctx context.Context, server string) error {
	...
	return fmt.Errorf(
		"deltachat: created chatmail account %q on %s. Update channel_list.deltachat.settings.email from %q to %q, remove the @server bootstrap marker, then run Rhizome again to use the account",
		addr,
		server,
		c.config.Email,
		addr,
...
func resolveDataDir(configured, channelName string) string {
	...
	return filepath.Join(home, ".rhizome", "deltachat", name)
}
