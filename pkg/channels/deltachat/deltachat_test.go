...
func TestNewDeltaChatChannel(t *testing.T) {
	...
	t.Run("missing email", func(t *testing.T) {
		...
		if !strings.Contains(err.Error(), "Next step:") || !strings.Contains(err.Error(), "rhizome g") {
...
func TestMentionsBot(t *testing.T) {
	tests := []struct {
		...
		{"multi word display name", "hey Rhizome Bot, can you help", "Rhizome Bot", "bot@example.org", true},
		...
		{"display name with prefix word", "hey SuperRhizome Bot", "Rhizome Bot", "bot@example.org", false},
...
func TestResolveDataDir(t *testing.T) {
	...
	home, _ := os.UserHomeDir()
	want := filepath.Join(home, ".rhizome", "deltachat", "mychan")
...
func TestEnsureAccountCreatesBootstrapAccountAndStops(t *testing.T) {
	...
	if !strings.Contains(err.Error(), "bot123@mehl.cloud") || !strings.Contains(err.Error(), "run Rhizome again") {
