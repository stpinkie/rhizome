...
import (
	...
	"github.com/stpinkie/rhizome/pkg/migrate/internal"
)

// OpenclawHomeEnvVar is the environment variable that overrides the source
// openclaw home directory when migrating from openclaw to rhizome.
// Default: ~/.openclaw
...
func (o *OpenclawHandler) ExecuteConfigMigration(srcConfigPath, dstConfigPath string) error {
	...
	picoCfg, warnings, err := openclawCfg.ConvertToRhizome(o.opts.SourceHome)
	if err != nil {
		return err
	}
...
func rewriteWorkspacePath(path string) string {
	path = strings.Replace(path, ".openclaw", ".rhizome", 1)
	return path
}
