...
func (c *OpenClawConfig) ConvertToRhizome(sourceHome string) (*RhizomeConfig, []string, error) {
	cfg := &RhizomeConfig{}
	var warnings []string
	...
	if c.HasSkills() {
		warnings = append(
			warnings,
			fmt.Sprintf(
				"Skills (%d entries) not automatically migrated - reinstall via rhizome CLI",
				len(c.Skills.Entries),
			),
		)
	}
	if c.HasMemory() {
		warnings = append(warnings, "Memory backend config not migrated - Rhizome uses SQLite with vector embeddings")
	}
	if c.HasCron() {
		warnings = append(
			warnings,
			"Cron job scheduling not supported in Rhizome - consider using external schedulers",
		)
	}
	if c.HasHooks() {
		warnings = append(warnings, "Webhook hooks not supported in Rhizome - use event system instead")
	}
	if c.HasSession() {
		warnings = append(warnings, "Session scope config differs - Rhizome uses per-agent sessions by default")
	}
...
func (c *OpenClawConfig) convertChannels(warnings *[]string) ChannelsConfig {
	...
	if c.Channels.Signal != nil {
		*warnings = append(*warnings, "Channel 'signal': No Rhizome adapter available")
	}
	if c.Channels.IRC != nil {
		*warnings = append(*warnings, "Channel 'irc': No Rhizome adapter available")
	}
	if c.Channels.Mattermost != nil {
		*warnings = append(*warnings, "Channel 'mattermost': No Rhizome adapter available")
	}
	...
	if c.Channels.BlueBubbles != nil {
		*warnings = append(
			*warnings,
			"Channel 'bluebubbles': No Rhizome adapter available - consider iMessage instead",
		)
	}
...
func (c *RhizomeConfig) ToStandardConfig() *config.Config {
