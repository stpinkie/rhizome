...
This package handles speech-to-text for Rhizome voice input.
...
Rhizome does not keep ASR API keys inside the `voice` section.
...
This is the recommended pattern because it is explicit, reusable, and consistent with the rest of Rhizome's model configuration.
...
- You can omit `api_base` and Rhizome will use Groq's default API base automatically.
...
Rhizome currently supports three main ASR routes:
...
## How Rhizome Chooses a Transcriber
...
2. If that resolved model is:
   - an `elevenlabs` provider model, Rhizome uses the ElevenLabs transcriber.
   - an OpenAI-compatible Whisper model, Rhizome uses the Whisper transcriber.
   - an audio-capable chat model, Rhizome uses `AudioModelTranscriber`.
3. **Fallback path**: if `voice.model_name` is not set, Rhizome performs a compatibility scan through `model_list` for legacy auto-detected ASR entries.
