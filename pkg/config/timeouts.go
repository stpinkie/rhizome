...
func DefaultTimeouts() TimeoutsConfig {
	return TimeoutsConfig{
		...
		Channel: ChannelTimeouts{
			...
			ToolFeedbackMaxDuration: Duration(10 * time.Minute),
...
// ChannelToolFeedbackMaxDuration returns the maximum lifetime of a single
// tool-feedback animation. A value <= 0 falls back to the default so a
// misconfiguration cannot re-enable unbounded editing.
func (c *Config) ChannelToolFeedbackMaxDuration() time.Duration {
	if c.Timeouts.Channel.ToolFeedbackMaxDuration > 0 {
		return c.Timeouts.Channel.ToolFeedbackMaxDuration.Duration()
	}
