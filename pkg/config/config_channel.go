...
func applyTelegramStreamingEnvCompat(target any) {
	...
	if raw, ok := os.LookupEnv("RHIZOME_CHANNELS_TELEGRAM_STREAMING_ENABLED"); ok {
		if value, err := strconv.ParseBool(raw); err == nil {
			settings.Streaming.Enabled = value
		}
	}
	if raw, ok := os.LookupEnv("RHIZOME_CHANNELS_TELEGRAM_STREAMING_THROTTLE_SECONDS"); ok {
		if value, err := strconv.Atoi(raw); err == nil {
			settings.Streaming.ThrottleSeconds = value
		}
	}
	if raw, ok := os.LookupEnv("RHIZOME_CHANNELS_TELEGRAM_STREAMING_MIN_GROWTH_CHARS"); ok {
		if value, err := strconv.Atoi(raw); err == nil {
