...
type GatewayConfig struct {
	Host      string `json:"host"                env:"RHIZOME_GATEWAY_HOST"`
	Port      int    `json:"port"                env:"RHIZOME_GATEWAY_PORT"`
	HotReload bool   `json:"hot_reload"          env:"RHIZOME_GATEWAY_HOT_RELOAD"`
	LogLevel  string `json:"log_level,omitempty" env:"RHIZOME_LOG_LEVEL"`
}
...
// The RHIZOME_LOG_LEVEL environment variable overrides the file value.
func ResolveGatewayLogLevel(path string) string {
	...
	if envLevel := os.Getenv("RHIZOME_LOG_LEVEL"); envLevel != "" {
