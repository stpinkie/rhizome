...
// RHIZOME_SSH_KEY_PATH to its path for the duration of the test. This is required
// whenever a test exercises encryption/decryption via credential.Encrypt or SaveConfig.
func mustSetupSSHKey(t *testing.T) {
	t.Helper()
	keyPath := filepath.Join(t.TempDir(), "rhizome_ed25519.key")
...
// TestSaveConfig_FiltersVirtualModels verifies that SaveConfig folds virtual
// models (generated by expandMultiKeyModels) back into their primary entry so
// the config file holds a single multi-key model, and that LoadConfig expands
...
func TestSaveConfig_FiltersVirtualModels(t *testing.T) {
	...
	// SaveConfig should fold the virtual model back into the primary
	if err := SaveConfig(path, cfg); err != nil {
		t.Fatalf("SaveConfig failed: %v", err)
	}

	// Reload and verify: the single multi-key entry expands back into
	// primary + virtual, so both keys survive the round trip.
	reloaded, err := LoadConfig(path)
	if err != nil {
		t.Fatalf("LoadConfig failed: %v", err)
	}

	if len(reloaded.ModelList) != 2 {
		t.Fatalf("expected 2 models after reload (primary + expanded virtual), got %d", len(reloaded.ModelList))
	}

	var primary, virtual *ModelConfig
	...
	if primary == nil {
		t.Fatal("expected primary model gpt-4 after reload")
	}
	if len(primary.APIKeys) != 1 || primary.APIKeys[0].String() != "key1" {
		t.Errorf("primary model should carry the first api_key, got %v", primary.APIKeys)
	}
	if virtual == nil {
		t.Fatal("expected expanded virtual model gpt-4__key_1 after reload")
	}
	if len(virtual.APIKeys) != 1 || virtual.APIKeys[0].String() != "key2" {
		t.Errorf("virtual model should carry the second api_key, got %v", virtual.APIKeys)
	}
  ...
  "hooks": {
    "processes": {
      "review-gate": {
        "enabled": true,
        "transport": "stdio",
        "command": ["uvx", "rhizome-hook-reviewer"],
        "dir": "/tmp/hooks",
        "env": {
          "HOOK_MODE": "rewrite"
        },
...
func TestDefaultConfig_WorkspacePath_Default(t *testing.T) {
	...
	cfg := DefaultConfig()
	want := filepath.Join(fakeHome, ".rhizome", "workspace")
...
func TestDefaultConfig_WorkspacePath_WithRhizomeHome(t *testing.T) {
	t.Setenv("RHIZOME_HOME", "/custom/rhizome/home")

	cfg := DefaultConfig()
	want := filepath.Join("/custom/rhizome/home", "workspace")

	if cfg.Agents.Defaults.Workspace != want {
		t.Errorf(
			"Workspace path with RHIZOME_HOME = %q, want %q",
...
// unchanged when RHIZOME_KEY_PASSPHRASE is not set.
func TestLoadConfig_NoSealWithoutPassphrase(t *testing.T) {
	...
	t.Setenv("RHIZOME_KEY_PASSPHRASE", "")
	t.Setenv("RHIZOME_SSH_KEY_PATH", "")
