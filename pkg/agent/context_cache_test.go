...
func setupWorkspace(t *testing.T, files map[string]string) string {
	t.Helper()
	tmpDir, err := os.MkdirTemp("", "rhizome-test-*")
	if err != nil {
		t.Fatal(err)
	}
...
func TestSingleSystemMessage(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			sys := msgs[0].Content
			if !strings.Contains(sys, "rhizome") {
...
func TestBuiltinSkillFileContentChange(t *testing.T) {
	...
	t.Setenv("RHIZOME_BUILTIN_SKILLS", builtinRoot)
	...
	for g := range goroutines {
		...
		go func(id int) {
			...
			for i := range iterations {
				...
				if !strings.Contains(result, "rhizome") {
...
func BenchmarkBuildMessagesWithCache(b *testing.B) {
	tmpDir, _ := os.MkdirTemp("", "rhizome-bench-*")
	defer os.RemoveAll(tmpDir)
