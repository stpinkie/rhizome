...
func (al *AgentLoop) runTurn(ctx context.Context, ts *turnState, pipeline *Pipeline) (turnResult, error) {
	...
	for ts.currentIteration() < ts.agent.MaxIterations || len(exec.pendingMessages) > 0 || func() bool {
		...
		if len(pendingMessages) > 0 {
			resolvedPending := resolveMediaRefsMode(
				pendingMessages,
				al.mediaStore,
				maxMediaSize,
				0,
				imageInlineModeForAgent(al.cfg, ts.agent, ""),
			)
			totalContentLen := 0
...
func (al *AgentLoop) askSideQuestion(
	...
	// Side-question model name is only known after candidate selection; use
	// the agent default (plus image_model fallback) for the inline decision.
	messages = resolveMediaRefsMode(
		messages,
		al.mediaStore,
		maxMediaSize,
		currentTurnStart,
		imageInlineModeForAgent(al.cfg, agent, agent.Model),
	)
