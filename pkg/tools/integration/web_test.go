...
// with cf-mitigated: challenge triggers a retry using the honest rhizome User-Agent,
...
func TestWebFetchTool_CloudflareChallenge_RetryWithHonestUA(t *testing.T) {
	...
	// Second request must use the honest rhizome user agent
	if !strings.Contains(receivedUAs[1], "rhizome") {
		t.Errorf("retry request UA = %q, want it to contain 'rhizome'", receivedUAs[1])
	}
}
