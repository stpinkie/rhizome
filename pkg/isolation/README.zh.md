...
`pkg/isolation` 为 `rhizome` 启动的子进程提供进程级隔离能力。

它当前不会把 `rhizome` 主进程自身放进沙箱中运行。
...
- 如果设置了 `RHIZOME_HOME`，使用该值。
- 否则默认使用用户目录下的 `.rhizome`。
...
隔离开启后，Rhizome 会打印生成后的隔离计划，便于排障。
